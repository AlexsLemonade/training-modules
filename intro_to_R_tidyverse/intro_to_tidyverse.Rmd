...
file `metadata.tsv` somewhere else using a relative `file.path()`.
* `setwd()` 
We will discuss and practice these concepts more in other modules as we go along.
```{r}
# The file we are reading in is our argument for read_tsv
metadata <- read_tsv("metadata.tsv")
```

Note that if we had not imported the tidyverse library, we would also need to 
tell R what package to find read_tsv in (because it is not a base R function).
To do this, we would use `::` to tell R to load in this function from the 
`readr` package. 
```
R_obj_name <- readr::read_tsv("FILE_NAME")
```
...
```{r}
all.equal(metadata_edit, metadata_edit2)
```
...
`select` function. 
```{r}
metadata %>% filter(age_mths > 100, sex == "M") %>%s
...
# Print out metadata_edit so we can look at what we made:
metadata_edit
```
...
```{r}
write_tsv(metadata_edit, "metadata_clean.tsv")
```
