...
Let's obtain the average expression value for each sample. 
```{r}
gene.data %>% summarize_all(funs(mean))
```
Now let's obtain the average expression by gene. 
We aren't doing this one by tidyverse, but just to show how you would get it: 
```{r}
gene.data$gene_mean <- apply(gene.data[, -1], 1, mean)
```
...
```{r}
write_tsv(gene.data, "gene_matrix_clean.tsv")
```
