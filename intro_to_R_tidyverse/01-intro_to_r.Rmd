...
*For more comprehensive tutorials:*  
  ...
  - For lot of handy R cheatsheets: https://www.rstudio.com/resources/cheatsheets/,
    including ones that show all the Rstudio  
  - For more guidance all the things you can do with R Notebooks:
...
comments on your code) if you put hashtags/pound signs.
...
Or, you can look in the `Environment` tab in the pane in the top right corner.
...
Based on the info we saw when we ran `ls()`, (or what you see in the 
`Environment` tab) what do you suppose will happen if we ask for R to print `y`?
...
Can you tell me what one divided by blue is? 
...
function so you can see how that author intended it to be used.
Alternatively, you can just search for the function by name in the *help bar*
in RStudio in the lower right corner.
A function's documentation should tell you what data *type* it will expect
as input.

### Different data types in R:

Let's explore different data types in R 
To find out what type of object `x` is, use the function `str`.
...
Right, by using quotes, we've made `x` a *character* variable. 
...
After running the above step, take a look at our environment. Is x there? 
...
If you want more or less rows, you can specify this by putting an `n = ` argument.

```{r}
tail(df, n = 3)
```

Why do you suppose we don't have a comma to subset this object?: 
...
#### 3) `sessionInfo` tells people what packages you used

This prints out what packages and versions you used in your analysis.
