...
Before we can import the file we need though, we should double check where R is 
...
Then we can use the `dir()` function to have R list the files in our current 
directory. 
...
Let's read in the file`metadata.tsv` that is in our current directory. 
...
exists in our current directory. 
...
```{r}
# Make a directory within the current directory called 'results'
dir.create("results")
```
...
Before we can save our file, we need to return to the concept of directories we
discussed at the beginning of this notebook. 
Remember that we created a `results` directory to put our output in. 
But if we want to save our data to a directory other than our current directory, 
...
from two examples below.

```{r}
# What is the difference among these file.paths? 
file.path("metadata_clean.tsv")
file.path("results", "metadata_clean.tsv")
file.path("metadata_clean.tsv", "results")
```

Which one of the three file.paths from above do we need to use in order to get our 
file into the `results` folder? 

Now that we have determined how to designate our file.path, we can write our 
`data.frame` to a file. 
To do this, we can use the `readr` library of `write` functions. 
In this case, we will use `write_tsv`.
Note that the second argument needs to be our a character string that contains 
our `file.path` that we determine above. 

Also note that in our notebooks, wherever you see a `<FILL_IN_THE_BLANK>` like
in the chunk below, that is meant for you to replace with the correct phrase
before you run the chunk (otherwise you will get an error).

```{r eval=FALSE}
write_tsv(metadata_edit, <PUT_CORRECT_FILE_PATH_HERE>)
```

For this example we have been working with `data.frame`s, however, we may also 
work with more complicated structure objects or very large objects. 
In these instances, we may want to save our data to an RDS file as opposed to 
a TSV like we did above. 
RDS is R's special file format for holding data exactly as you have it in your 
R environment. 
RDS files also are compressed, meaning they will take up less space on your 
computer. 
Just like when we made the file.paths above, we will want to end the file.name 
in the correct extension so here, we will use a similar `file.path` as we did 
before, but replace the `.tsv` with `.RDS`

```{r}
# Write your object to an RDS file
write_rds(metadata_edit, file.path("results", "metadata_clean.RDS"))
```
...
RDS file is called? 
Use that function here to reimport your data in the chunk we set up for you
below.
