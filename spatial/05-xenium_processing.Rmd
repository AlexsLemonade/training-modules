...
We only measure the genes on the panel (here, 313 genes), so we can't discover expression of genes that weren't targeted.
As a tradeoff, we get very high sensitivity and spatial precision for the genes we do measure.
...
We'll define the path to both the Xenium output directory and the annotations file here.
...
The same data are provided in HDF5 (`.h5`), a Matrix Market directory, and Zarr formats; we will we use the `.h5` version.
...
An H&E image can added to the object downstream, but we won't be working with one with this dataset.
...
# and matching the order in the palette
...
Where in the sample do the outliers appear?
...
spe$control_exclude <- spe$control_probe_counts > 0 |
...
This will ensure that the differences in total counts across cells don't dominate downstream analyses, and is the same normalization we used with Visium data.
...
Finally, we will save the filtered, normalized `SpatialExperiment` object as an `.rds` file so it can be loaded directly into later notebooks without repeating the steps above.

```{r export-spe}
readr::write_rds(normalized_spe, output_spe_file)
```
We will also save the color palette as a table.
...
To conclude, we'll run the `sessionInfo()` command to print out exactly what system settings and R package versions were used to run this code.
