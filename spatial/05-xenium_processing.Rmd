...
- Resolution is subcellular and truly single-cell.
...
```{r celltype-palette}
# build a named color palette for the lumped cell types
# we have 14 cell types and alphabet has 26 distinct colors
cell_type_levels <- sort(unique(spe$cell_types_lumped))
cell_type_palette <- setNames(
  # reverse the order so cell types that are close together in space are distinct
  rev(palette.colors(palette = "alphabet")),
  cell_type_levels
)
...
`SpaceTrooper` needs to know which technology produced the data, which we set in the object's `metadata()` first.
...
spe <- SpaceTrooper::spatialPerCellQC(spe)
...
```{r plot-qc}
p1 <- ggspavis::plotCoords(spe, annotate = "Area_um") +
  scale_color_viridis_c()

p2 <- ggspavis::plotCoords(spe, annotate = "total") +
  scale_color_viridis_c()
...
# (3 median absolute deviations below the median, on the log2 scale)
...
```{r plot-remove}
# store the flag and visualize which cells will be removed
spe$remove <- exclude

ggspavis::plotCoords(spe, annotate = "remove") +
  guides(color = guide_legend(override.aes = list(size = 4)))
```

```{r filter-cells, live = TRUE}
# keep only the cells not flagged for removal
filtered_spe <- spe[, !spe$remove]
