...
We can answer this question more precisely with a heatmap.
For this, we'll consider again the somewhat conservative `0.1` threshold for calling a cell type as present in each spot.
...
# create a co-occurrence data frame
cooccur_df <- present_df |>
  ...
  dplyr::count(cell_type_a, cell_type_b) |>
  # normalize to get the proportion of spots they co-occur in
  dplyr::mutate(prop_spots = n / ncol(spe))

# plot a heatmap
ggplot(cooccur_df) + 
  aes(x = cell_type_a, y = cell_type_b, fill = prop_spots) +
  geom_tile() +
  geom_text(
    aes(label = round(prop_spots, 2)),
    color = "black", 
    fontface = "bold"
  ) +
  ...
  labs(
    x = "", 
    y = "", 
    fill = "Proportion of spots"
  ) +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 30, hjust = 1))
```
We see for example that tumor and immune myeloid cells co-occur in about 47% of spots, assuming a 0.1 threshold.
