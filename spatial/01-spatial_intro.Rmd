...
Even though spots aren't exactly cells, we can still color by gene expression to get a general sense of patterns in our sample.
...
We can see some of this biology reflected in this tissue section that appears to have these two major components: bluer regions indicating ECM-poor areas with high cellular density consistent with blastemal biology, and paler pink regions consistent with stromal tissue.

We've picked out a couple genes of interest to color by:
...
To start, let's color by `WT1` expression.
We expect this to be expressed across developing kidney cells and in particular malignant cells. 

```{r color-wt1, live = TRUE}
gene_index <- 1

wt1_plot <- ggspavis::plotCoords(
  norm_spe, 
  # name of variable to color by
  annotate = marker_genes[gene_index],
  # counts are plotted by default, so we need to override this
  assay = "logcounts", 
  point_size = 1
) + 
  ggplot2::scale_color_viridis_c() +
  ggplot2::labs(title = names(marker_genes[gene_index]))
...
Notice that little yellow band inside the dark region on top that matches nicely with the tissue image?
It's very satisfying.

Let's compare this to `COL1A1` expression (the 5th gene), which we expect to be more prevalent in stromal regions.
...
We have a few more genes listed in the table, take a moment to plot one below.
...
# include H&E in plot
patchwork::wrap_plots(c(list(he_plot), plot_list), nrow = 3)
```
All of this taken together, which regions likely contain malignant vs normal cells?
