...
While the `raw_feature_bc_matrix` contains data for all possible spots, but the `filtered_feature_bc_matrix` contains data only for spots that overlap tissue. 

Parallel to these directories is the `spatial` directory which contains - you guessed it - files that provide the full spatial context to go along with the feature-by-barcode count matrices. 
...
Below are some brief descriptions of each file we see here, and more that would be represent with more recent Visium technologies.
...
So, we'll just specify to import the `lowres` version of the image.
...
First is a dedicated `spatialCoords` slot which contains the x/y coordinates on the slide in pixel units.
