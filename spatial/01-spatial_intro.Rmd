...
# To support plotting
library(patchwork)
# library(eurlerr) TODO: uncomment (needs to be loaded since it uses plot())
...
We also gain some insight about how global vs local filtering.
...
Protip!
When using this function, the point color is a "fill" aesthetic (which can be customized), and the point outline is a "color" aesthetic (the color of which is quite finicky to customize so we'll leave that alone, but we can customize its linewidth).

```{r spotsweeper-plotqc-sum, live = TRUE}
#| fig.width: 10
#| message: FALSE

log_plot <- SpotSweeper::plotQCmetrics(
  filtered_spe, 
...
z_plot <- SpotSweeper::plotQCmetrics(
  filtered_spe, 
...
What patterns do you see between these plots?
...
```{r spotsweeper-large-k}
#| fig.width: 10
#| message: FALSE

# Detect outliers with a very large neighborhood and make associated plot
...
# Detect outliers with a very small neighborhood and make associated plot
...
```
We can see the relationship more clearly with some scatterplots:
...
The top left shows spots that will only be filtered out for `k = 100`, and the bottom right shows spots that will only be filtered out for `k = 10`.
...
Let's get back on track: let's do the same outlier detection for `detected` and `subsets_mito_percent`, again using the default cutoff of 3 with the default neighborhood size of 36.
...
# TODO: EVAL THIS CHUNK ONCE THE PACKAGE GOES IN
eulerr_data <- eulerr::euler(outliers_df)

plot(
  eulerr_data, 
  quantities = TRUE, 
  # TODO: Can pick a style in review
  legend = TRUE,
  labels = TRUE,
