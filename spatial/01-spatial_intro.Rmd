...
This sample was processed using first-generation Visium technology, and it was quantified with [`Space Ranger 1.3.1` from 10x Genomics](https://www.10xgenomics.com/support/software/space-ranger/latest).
...
We've placed the core outputs you'll need for reading this first-generation Visium data into R in the directory `data/wilms-tumor/SCPCS000190`, organized into folders as Space Ranger returns them.
...
Because the data we're working with comes from the [ScPCA Portal](https://scpca.alexslemonade.org/), which only provides the folder format, that's what we'll read in here.
...
By default, this function will try to import several image types, including the CytAssist image which we don't have here.
...
ggspavis::plotVisium(
  ...
  # use guides() to override legend titles in ggspavis
  guides(
    fill = guide_legend(
...
However, we'll learn in the following sections that we interpret and use these statistics somewhat differently from single-cell data.
...
qc_plot_sum <- ggplot(coldata_df) +
  aes(x = sum) +
  geom_density(fill = "lightblue") +
  labs(title = "Total unique UMIs")

qc_plot_detected <- ggplot(coldata_df) +
  aes(x = detected) +
  geom_density(fill = "steelblue") +
  labs(title = "Total detected genes")

qc_plot_mito <- ggplot(coldata_df) +
...
To see their full spatial context, we'll plot those same QC statistics directly on the slide coordinates.
...
# plots for each QC measure
qc_spots_sum <- ggspavis::plotCoords(
  filtered_spe,
  annotate = "sum",
  point_size = 1
) +
...
qc_spots_detected <- ggspavis::plotCoords(
  filtered_spe,
  annotate = "detected",
  point_size = 1
) +
...
qc_spots_mito <- ggspavis::plotCoords(
  filtered_spe,
...
Their hangnail detection uses variance in mitochondrial percentages, and since our mitochondrial percentages are very low with minimal variance, hangnail detection won't be relevant for us here.
...
ggplot(filtered_spe_coldata) +
  ...
  # add a line for the cutoff value
  geom_hline(yintercept = -3) +
  # and some labels
  labs(
    x = "log(sum + 1)",
    y = "Local z-score of sum value",
    color = "sum outlier"
  )
```

We see here that most spots are not outliers for the `sum` metric.
Most of the spots classified as outliers are fairly close to the -3 z-score cutoff, but several are more extreme outliers.
...
We can use this knowledge to help us to customize the plot colors.
...
sum_z_plot <- SpotSweeper::plotQCmetrics(
  ...
  scale_color_manual(values = c(`TRUE` = "black", `FALSE` = "transparent")) +
  labs(title = "Local z-score for total UMI")
...
SpotSweeper::plotQCmetrics(
  ...
  scale_color_manual(values = c(`TRUE` = "black", `FALSE` = "transparent")) +
  scale_fill_viridis_c() +
  labs(title = "Log of total genes detected")
```
...
filtered_spe <- filtered_spe[, !filtered_spe$local_outliers]
...
We'll therefore use a so-called ["library-size normalization approach"](https://bioconductor.org/books/3.22/OSTA/pages/seq-intermediate-processing.html#library-size-normalization) which treats each spot fully independently.
...
wt1_plot <- ggspavis::plotCoords(
  ...
  scale_color_viridis_c() + # custom color scale
  labs(title = "WT1")
...
readr::write_rds(norm_spe, output_spe_file, compress = "gz")
```
