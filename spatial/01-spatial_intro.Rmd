...
All this means is: Keep your peeled as the spatial transcriptimics analysis field matures, because we're still in the early days of method development.

Either way, we'll now proceed to normalize using `scuttle::computeLibraryFactors()` to calculate our spot-specific size factors.
...
Spatial data, by definition, already has a handy coordinate system already which gives us a meaningful layout for visualizing spatial data.
...
What differences do you see this plot of `COL1A1` gene expression compared to `WT1`?
