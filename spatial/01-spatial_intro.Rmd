...
To begin, we'll load the core library for spatial transcriptomic objects in Bioconductor, `SpatialExperiment`, as well as some packages for plotting.
We won't be doing any calculations involving random numbers here, so we don't need to set a random seed as part of setup (but we will need to later in the workshop).
...
We've placed the core outputs you'll need for reading this Visium v1 data into R in the directory `data/wilms-tumor/SCPCS000190/outs`, organized just as `Space Ranger` returns them.
...
# we'll use of the existing sample_id variable to write the file name
...
`Space Ranger` output includes both count data and spatial/image information. 
The count data is provided in two main formats, just as `Cell Ranger` would provide them for single-cell data: 1) A folder with a feature list, a barcode list, and a sparse matrix in ["Matrix Exchange" format](https://math.nist.gov/MatrixMarket/formats.html), and 2) an `HDF5` formatted file containing all this information.
...
`Space Ranger` also outputs both filtered and raw matrices; today we will start with the raw matrix and perform our own filtering.

We'll get started by reading in the Visium data directly from its `Space Ranger` output, using the package `VisiumIO`.
Before reading it in, let's go ahead and look at our files:
...
In a Visium dataset such as this, the `raw_feature_bc_matrix` contains data for all possible spots, but the `filtered_feature_bc_matrix` contains data only for spots that overlap tissue. 

Parallel to these directories is the `spatial` directory which contains - you guessed it - information that proves the full spatial context to go along with the feature-by-barcode count matrices. 
...
Below are some brief descriptions of each file we see here.
...
This is because those spots will have 0 gene expression, but that doesn't mean that genes actuallt weren't expressed!
...
These are two different versions of the provided tissue image that have each been downscaled for size to, respectively, ~2000 pixels and ~600 pixels.
...
In other Visium datasets, you may see more files in `spatial/` than you see here.
In particular, this data was prepared with the original Visium technology, in which tissue was manually placed directly onto a Visium capture slide.
...
- `aligned_tissue_image.jpg`: If you provided a microscope image along with the CytAssist image to a CytAssist run, `Space Ranger` will also return the microscope image aligned to the CytAssist image's coordinates

### Import with VisiumIO

We'll begin by importing the raw counts and spatial information as a `SpatialExperiment` (SPE) object.
...
To import Visium data with the `VisiumIO` package, you need to specify three items (the rest should be automatically detected):
...
By default, this function will try to import all types of possible images, including the CytAssist image which we don't have here! 
So, we'll just specify to import the `lowres` version of the image which will keep this notebook size smaller.
...
Like SCE objects, this SPE objeect has slots like `assays` for count matrices, `rowData` for feature metadata, and `colData` for spot metadata.
...
Rows with 0 correspond to spots that do _not_ overlap tissue, so we'll need to filter those out before processing or analyzing this data further. 
...
_Unlike_ SCE objects, SPE objects contain two additional slots with spatial information.

First is a dedicated `spatialCoords` slot which contains the x/y coordinates on the slide represented as pixels.
...
We can also plot just the coordinates themselves with `ggspavis::plotCoords()`.
...
Now, this was only two genes, but more pretty plots is more, and in this case better.
Here's a table of genes of interest we might want to plot:

| Gene name | Description                                  |
| --------- | -------------------------------------------- |
| `WT1`     | Developing kidney marker                     |
| `MYCN`    | malignant marker                             |
| `NCAM1`   | malignant blastema compartment marker        |
| `SIX1`    | malignant blastema compartment marker        |
| `COL1A1`  | fibroblast/mesenchymal compartment marker    |
| `TAGLN`   | myofibroblast/mesenchymal compartment marker |
| `PECAM1`  | epithelial compartment marker                |

Let's see how we can automate plotting all of this at once using some `purrr::map()` magic.


First, we'll define a named list of Ensembl ids, named with their gene names, for use in plotting.

```{r define-gois}
# define vector of genes of interest
goi_vector <- c(
  "WT1",
  "SIX1",
  "MYCN",
...
# create named vector with ensembl ids as values values
named_goi_list <- goi_vector |>
  # set names of this vector as the vector values themselves
  purrr::set_names() |>
  # extract ensembl for each gene symbol
  purrr::map(
    \(gene_symbol) rownames(norm_spe)[rowData(norm_spe)$Symbol == gene_symbol]
  )

# show the named list of genes
named_goi_list
```
...
plot_list <- named_goi_list |>
  # map over value and name at once
  purrr::imap(
...
# plot coordindates for all genes, including the H&E 
patchwork::wrap_plots(
  c( list(he_plot), plot_list ), 
  nrow = 2
)
```
All of this taken together, do you think there is evidence for blastemal and stromal compartments in this sample?
Based on this, which regions do you think likely contain malignant vs normal cells?
