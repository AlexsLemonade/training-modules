...
We'll use real gene expression dataset to get comfortable making visualizations using ggplot2. 
...
+ A list of [ggplot2 courses and books from Tidyverse](https://ggplot2.tidyverse.org/)  
+ The [ggplot2 data viz chapter of R for Data Science](https://r4ds.had.co.nz/data-visualisation.html)  
+ A [ggplot2 online tutorial](http://r-statistics.co/Complete-Ggplot2-Tutorial-Part1-With-R-Code.html)  
...
```{r}
stats_df <- mutate(stats_df, neg_log_p_val = -log10(adj_p_value))
```
...
```{r}
dplyr::filter(stats_df, contrast == "astrocytoma_normal") 
```

Let's save this as a separate data.frame by assigning it a new name. 

```{r}
tumor_normal_df <- dplyr::filter(stats_df, contrast == "astrocytoma_normal") 
```
...
ggplot(tumor_normal_df, 
  ...
  theme_classic() # Add on this set of aesthetics
```
...
ggplot(tumor_normal_df, 
  ...
  ylab("-log10 Adjusted p value") # Add a y label
...
Note we have to switch back to `stats_df` which contains the stats for all three contrasts. 
