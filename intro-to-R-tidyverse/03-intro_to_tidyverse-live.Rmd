...
If we try to coerce it into a matrix using `as.matrix()`, R will have to pick a type that it can convert everything to. What does it choose?
...
While that worked, it is rare that we want numbers converted to text, so we are going to select only the columns with numeric values.
We can do this most easily by removing the first column, as we did in `apply()` function examples above.

```{r matrix-numeric, live = TRUE}
# Let's save a new matrix object names `gene_num_matrix` containing only the numeric values

# Explore the structure of the `gene_num_matrix` object

```

Now that the matrix is all numbers, we can do things like calculate the column or row statistics using `apply()` as we did above.
...
Note that the length and the structure of the `gene_matrix_means` object is the same as the length and structure of the `gene_means` object which we generated using the `gene_df` data frame.
The difference here is that the `gene_means` data frame object would contain a combination of character and numeric data types if we did not remove the first column, while the `gene_matrix_means` object would contain only one data type, character, if we did not remove the first column because we coerced the object into a matrix.
