...
A matrix might at first appear similar to a data frame, but it has an additional
constraint: rather than each column having a single type, ALL data in the matrix
has the same type.

The data frame above is made up of two different types, character and numeric,
so converting it to a matrix would require us to make it all the same type.
If we try to coerce it into a matrix using `as.matrix()`, R will have to pick a
type that it can convert everything to.
What does it choose?
...
We can do this most easily by removing the first column, as we did in `apply()`
...
or row statistics using `apply()` as we did above.

```{r matrix-rowmeans}
# Calculate row means for our numeric matrix object
gene_matrix_means <- apply(gene_num_matrix, 1, mean)
length(gene_matrix_means)
str(gene_matrix_means)
```

Note that the length and the structure of the `gene_matrix_means` object is the
same as the length and structure of the `gene_means` object which we generated
using the `gene_df` data frame.

The difference here is that the `gene_means` data frame object would contain a
combination of character and numeric data types if we did not remove the first
column, while the `gene_matrix_means` object would contain only one data type,
character, if we did not remove the first column because we coerced the object
into a matrix.

We can put the character gene names back into the numeric matrix object by
