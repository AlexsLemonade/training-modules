...
Note that we can obtain the same results using the columns with numeric values
from the `gene_df` data frame.

```{r rowmeans-dataframe}
# Calculate row means using the `gene_df` object
gene_means_from_df <- apply(gene_df[, -1], 1, mean) # Removing the character
# column allows R to do the as.matrix() coercion automatically, which only works
# if the rest of the data frame is numeric.
