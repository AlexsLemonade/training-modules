...
But, as we've also seen, this function will throw an error if you try to create a directory that already exists, which can be frustrating if you are re-running code!
A different option is to use the [`fs`](https://fs.r-lib.org/) package, which provides functions for you to interact with your computer's file system, and _bonus_ it's guaranteed to work on any kind of operating system (Max, Linuc, PC, etc.)!
...
- It allows you to quickly create _nested_ directories, i.e. in one call make directories inside of other directories
