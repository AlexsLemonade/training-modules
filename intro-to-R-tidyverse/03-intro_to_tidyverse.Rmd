...
In our Docker container, we already have the `tidyverse` and other packages we will
...
`GSE44971.tsv` that is in the `data` folder to a data frame named `gene_df`. 

```{r eval = FALSE}
# Use this chunk to read in data from the file `GSE44971.tsv`
gene_df <- <YOUR_CODE_HERE>
```
...
```{r include=FALSE}
# in case gene_df wasn't loaded yet
