...
A matrix is similar to a data frame in that it is a rectangular table of data, but it has an additional constraint: 
rather than each column having a type, ALL data in a matrix has the same type.

The data frame above has  columns of two different types, character and numeric, but converting it to a matrix will require us to make them all the same type.
We can still coerce it into a matrix using `as.matrix()`, in which case R will pick a type that it can convert everything to.
What does it choose?
