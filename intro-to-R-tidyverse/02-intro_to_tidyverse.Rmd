...
Now that we've determined this file exists, we are ready to read it into our R
environment
...
Recall that `metadata_pipe` and `metadata` are data.frames with information about some of the same samples.
...
For now, we will focus on `inner_join`, which is used when you 
only want to keep an intersection of two data.frames. 
We need to use the `by` argument to designate what column(s) 
should be used as a key to match the data.frames. 

```{r}
metadata %>% inner_join(metadata_pipe, by = 'sample_id') 
```

Note that if there are duplicate columns, the default is to save both columns 
but a `x` or `y` is added to the name to note which data.frame it originated from.
...
The two basic arguments needed by `ggplot` are 1) `data` and 2) `aes` (short for
aesthetics).
...
The variables we specify should be named columns within the object we specifed as 
our `data`. 
...
There is a lot more we could do to further personalize this plot, but for now
