...
R itself is an _interpreted computer language_ with a full set of programming features, which we refer to as **"base R"** - the functionality that comes bundled with the language itself. 
There is rich additional functionality provided by **external packages**, or libraries of code that assist in accomplishing certain tasks. These packages, also referred to as libraries, can be freely downloaded and loaded for use. 

We will focus on using a suite of packages collectively known as [**The Tidyverse**](http://tidyverse.org). The `tidyverse` is geared towards intuitive data science applications that follow a shared data philosophy.
...
![RStudio Appearance](screenshots/rstudio-cloud-anatomy.png) 
...
R scripts can also directly be run from this pane. 
In this image, you can see an R script called "example.R" which contains some simple R to define and print a variable.

2. The **console** pane is where you can _interactively_ run R code. 
You can also use this pane as a **terminal** to emulate a UNIX shell (please ignore this if you are unfamiliar with UNIX). 
In this image, you can see that the user has defined a variable called "y" and printed out its value. 
In the console, new lines where you can type code are indicated by greater than signs `>` - we do not type these symbols - R places them there to indicate a new line is starting.

3. The **environment** pane primarily displays the variables, sometimes known as _objects_ you define during a given R session. 
You can ignore its other tabs for now. In this image, you can see that a variable "y" has been created with the value "9" (see the "console" code?). 
You'll also notice that the variable "x" is _not_ listed in the environment pane - that's because the script "example.R" has only been written but not (yet!) run, so R doesn't know about it.
...
We will begin with _interactive coding_ to explore R's functionality. 
Launch your own new project in RStudio Cloud and answer the following questions by writing simple operations in the **console**.
...
Anything on a given line that appears after a hashtag will be ignored, whether the hashtag is at the beginning of a line or in the middle of a line. 
You will see many comments in example code below!
...
This variable's name is `x`, its value is `7`, and its type is `numeric` (7 is a number!). We will learn more about variable types soon.
 + Variable names can be any _string_, i.e. a set of characters (numbers, letters, and cerain symbols).
Variables names should start with a letter, and it is best practice to avoid all symbols EXCEPT underscores. 
**Never use periods, dollar signs, or spaces. If you do, you will learn this the hard way.**
...
Perform the following computations using functions. 
Try to use _variables_ while you code!
...
Now that you have a general sense of R usage, let's learn about the different types of variables. 
We have already seen "numeric" types above using numbers, but there are more than just numbers out there. 
...
# Try to perform calculations with `x`, what happens? 
```
...
This is because, in fact, all (ok mostly all) variables are _by default_  vectors, and our answers are the first (in these cases only) value in the array. 
...
We can define arrays with the function `c()`, which stands for "combine". 
This function takes a comma-separated set of values to place in the array, and returns the array itself:
...
We can pull out specific items from an array using a process called _indexing_, which uses brackets `[]` to specify the position of an item. 
...
One major benefit of arrays is the concept of **vectorization**, where R by default performs operations on the _entire array at once_. 
...
```{r}
example_vector <- 2:8 # Define an array of values ranging from 2-8
print(example_vector)

## Which values are <= 3?
## The output here is a LOGICAL VECTOR telling us whether each value in example_vector is TRUE or FALSE
print(example_vector <= 3)
```

### A note on variable naming
Choosing variable names that are clear indicators of what the variable is, is not only helpful for yourself, but also 
We have learned functions such as `c`, `length`, `sum`, `sqrt`, and more. 
Imagine defining a variable called `c`: This will work, but it will lead to a LOT OF UNINTENDED BUGS. **CHOOSE YOUR VARIABLE NAMES WISELY!**
...
# My vector where my vector is greater than 10
my_vector[my_vector > 10]  # get the values that are greater than 10
```
    + Subtract 10 from all values in `numbers`, and save the new vector as a _different variable_ called `numbers_minus10`. You should now have TWO defined vectors!
    + Square root all values in each of your two vectors using the `sqrt()` function, and save the output from these commands respectively as `numbers_sqrt` (output from square rooting `numbers`) and `numbers_minus10_sqrt` (output from square rooting `numbers_minus10`). Print your new variables out. What do you see and why? Take a moment to really understand any unexpected output.
    ...
    + There are several key functions which can be used on vectors containing numeric values, some of which are below. Run each of these functions on the vector `numbers_sqrt` and `numbers_minus10_sqrt`, and compare the outputs. What does this tell you about how R deals with NA or NaN values?
    ...
    + We can also use the `sum()` function with _logical expressions_, as shown in the example below. Use the `sum()` function to determine how many values in ``numbers_sqrt` are greater than 2.
      ```{r, collapse=TRUE}
      my_vector <- c(1,4,6,32,26)
      sum(my_vector > 10)  # how many values are greater than 10?
      ```
2. Define a _logical_ vector as `logical_vector <- c(TRUE, TRUE, TRUE, FALSE, FALSE, FALSE, TRUE)` (NOTE: you can write `TRUE` or `T` but never `true`, `True`, or `t`. In fact, `t()` is a totally separate function for transposing matrices that we will not be using.)
   + Use the `sum()` function to add up this vector - you will get an actual answer! Where does this answer come from? (Hint: Try coercing the value `TRUE` to be an integer with `as.integer(TRUE)`, and similarly `FALSE`. This logical-numerical mapping is standard across all computer programming languages).
   
   + The function `which()` is sort of the opposite of indexing: It tells you at which index/indices a _logical condition_ is satified. 
   This function takes a single argument, the _logical expression_ of interest (see example below) and returns an vector. 
   Use the `which()` function to determine which values in `logical_vector` are `TRUE`. 
      ```{r, collapse=TRUE}
      temp_vector <- c(4,8,2,34,5,8)
      which(temp_vector == 8)
      which(temp_vector > 50) ## returns an empty vector since nothing is greater than 50
      ```
...
3. Define two character variables as `one <- "first"` and `two <- "second"`. Perform the following tasks with these two variables:
    ...
    + Create an vector called `both` with TWO values, `"first"` and `"second"`
    + Use indexing AND logical operations to determine if the first or second index of `both` is longer
    + Create a new variable with the value `"firstsecond"`. 
    Then, add this string as a _third_ item in the vector. 
      > Hint: To merge strings, use the `paste0()` function. Look it up and try it out!
    + Create a new variable with the value `"first!second"`. 
    Then, add this string as a _fourth_ item in the vector. 
      > Hint: To merge strings with a specified _separator_ (here, `!`), use the `paste()` function. Look it up and try it out!
    
4. Define this vector: `new_vector <- c("a", "b", "c", "d", 5, 6, 7, "hijk")`
    + The `%in%` logical operator is useful for determing whether a given item is in an vector. Use this operator to determine if `"a"` is in `new_vector`
    + Use the `%in%` operator to determine if `7` is in `new_vector`
    + Use the `%in%` operator to determine if `"h"` is in `new_vector`. Did you expect this answer? Why or why not?
...
The type `data.frame` is itself a datatype in R, but we will (soon) be using a related datatype called a `tibble`: This is effectively a data frame within the tidyverse framework that just has a couple features that make it easier to use than a regular old data frame.
...
## We can perform our regular vector operations on columns directly, e.g:
