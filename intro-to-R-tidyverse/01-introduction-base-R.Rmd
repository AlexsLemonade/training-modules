...
Here we can specify an _argument_ of `base` to make this base 3 log. 

```{r}
log(81, base = 3)
```

If we don't specify the _argument_ names exactly, it assumes they are in order of what `log` has them. 
...
We can provide named objects just the same. 
...
We often want to compare different quantities, and to do this we will use a special set of comparison operators. 
Each of these operators asks a particular question about the values on either side of it, such as "Are these equal?" or "Is the left greater than the right?". 
They answer with a _logical variable_ (known as "boolean" in many other languages), which is either `TRUE`, `FALSE` or `NA` (for when the question can't be answered).

| Operator | What it does |
|----------|--------------|
 `==`           |  Tests if two quantities are equal |
  `>` and `<`   |  Tests if one quantity is greater than or less than another |
  `>` and `<`   |  Tests if the left quantity is greater than or less than the one on the right |
...
!(4 > count)  ## 4 is not greater than 8. Parenthesis part is FALSE, but the `!` changes it to TRUE
```
