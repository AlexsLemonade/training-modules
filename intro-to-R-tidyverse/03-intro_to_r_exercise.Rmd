...
Use the template from above to make `gene_b` and a `gene_c`. 
...
Write these data to their own TSV file using the `readr::read_tsv` function
...
Use the `==` operator to check the `gene_means` individually by subsetting each 
gene using `mouse_data$`.  
