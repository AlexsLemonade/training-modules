...
Here, 
TODO: Determine suitable experiment and write about it

### Setup 
Before we start the exercises, we need to create the necessary directories and loading the necessaty libraries.
...
```{r}
# Reading Metadata
metadata <- read_tsv(file.path("exercise-data","metadata_GSE49243.tsv"))

# Reading Gene Expression Matrix
gene_expression <- read_tsv(file.path("exercise-data","GSE49243.tsv"))
```
...
metadata_clean <- metadata %>% 
                  filter(refinebio_age < 18) %>% 
                  select(refinebio_accession_code, refinebio_age, refinebio_sex, histology) %>%
                  arrange(refinebio_age)
```


#### Rename columns
The coloumn names `refinebio_accession_code`, `refinebio_age`, `refinebio_sex` are too unwieldly. Let's rename it to something more manageable like `sample_id`, `age`, and `sex` respectively. 
...
metadata_clean <- metadata_clean %>%
                  rename(sample_id = refinebio_accession_code, age = refinebio_age, sex = refinebio_sex)
...
metadata_clean %>% 
  group_by(histology,sex) %>% 
  summarize(counts = length(histology))

```


### Filter the Gene Experssion Matrix
Now, filter the gene expression matrix so that it has the same samples as `metadata_clean`. (Remember we read the gene expression matrix into `gene_expression`)

```{r}
filtered_gene_expression <- gene_expression %>%
                            select(metadata_clean$sample_id)
```
...
Last step, write the cleaned metadata into `metadata_GSE49243_cleaned.tsv` and filtered gene expression matrix into `GSE49243_filtered.tsv` the exercise-results directory. 

```{r}
write_tsv(metadata_clean, file.path("exercise-results","metadata_GSE49243_cleaned.tsv"))
write_tsv(filtered_gene_expression, file.path("exercise-results","GSE49243_filtered.tsv"))
```

You should notice two files named `metadata_GSE49243_cleaned.tsv`and `GSE49243_filtered.tsv has been created in `exercise-results` directory. 
...
#### Read Gene expression Matrix and cleaned metadata
...
```{r}
gene_means <- apply(filtered_gene_expression, 1, mean)
```
