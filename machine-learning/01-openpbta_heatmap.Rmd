...
It is highly flexible; see the [`ComplexHeatmap` Complete Reference](https://jokergoo.github.io/ComplexHeatmap-reference/book/) for more information.
...
We store the data we'll use in this notebook in `data/open-pbta/processed`. 
...
If we make a heatmap to get an idea of the structure in our data, it can be helpful to subset to high variance genes.
...
The names of the list need to exactly match the column names in `sample_annotation_df` and the names in each vector need to exact match the values in those columns.
...
This value isn't exact zero but it's very close!
...
That means all the variance values should be equal to one, too.
...
We'll use Pearson correlation for our distance metric here again, but let's breakdown what goes into hierarchical clustering a bit here.
...
Hierarchical clustering is performed on dissimilarities, so we'll subtract the correlation values from 1.
...
Now we're ready to perform hierarchical clustering with `hclust()`.
