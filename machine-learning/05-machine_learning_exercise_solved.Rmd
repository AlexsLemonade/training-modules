...
data_dir <- file.path("data", "GSE116436", "processed")
...
# Metadata file
metadata_file <- file.path(data_dir, "metadata_GSE116436_cleaned.tsv")
# PLIER file
plier_file <- file.path(models_dir, "NCI60_PLIER_model.RDS")
```
...
png_file <- file.path(plots_dir, "NCI60_lineplot.png")
```
...
summary_df <- plier_results$summary
```
...
proteasome_df <- data.frame(geo_accession = colnames(plier_results$B),
                            proteasome_lv = plier_results$B[19, ])
```
...
treatment_plot + 
  ...
  labs(y = "Proteasome LV Expression Level",
       x = "Time (hours)",
       title = "Bortezomib")
```
...
proteasome_meta_df <- proteasome_meta_df %>%
  mutate(time_hours = as.factor(time_hours))
```
...
treatment_plot_factor <- proteasome_meta_df %>%
  filter(treatment == "bortezomib",
         tissue == "Lung") %>% 
...
ggsave(png_file, plot = last_plot())
```
