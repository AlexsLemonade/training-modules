...
Here's a handy [ggplot2 cheatsheet to help](https://www.rstudio.com/wp-content/uploads/2015/03/ggplot2-cheatsheet.pdf).
...
```{r}
lv_plot <- file.path(plots_dir, "LV13_boxplot.pdf")
```
...
```{r}
lv <- "13,SVM T cells CD4 memory activated"
```
...
lv13_df %>%
  ...
  # add built in theme
  theme_classic() +
  labs(x = "Subgroup", y = "LV Expression", title = "LV13 CD4 T Cells Memory Activated")
```
...
```{r}
pairwise.wilcox.test(x = lv13_df$Expression_level, g = lv13_df$subgroup,
...
lv13_df %>%
  ...
  theme(plot.title = element_text(hjust = 0.5, face = "bold"),
        ...
        legend.position = "none") +
  scale_colour_manual(values = c("#FFC125", "#00CD66", "#00BFFF")) +
  geom_signif(y_position = c(1.25, 1.45, 1.65), 
              xmin = c("Group 3", "Group 3", "Group 4"), 
              xmax = c("Group 4", "SHH", "SHH"),
              annotation = c("0.38375", "0.00044", "9.1e-05"), 
