...
2) **FASTA files** that contain sequence information 
...
It will also conveniently include these sequence ranges in the `tximeta` object should you have an analysis that might need them. 

We can obtain these FASTA files from Ensembl's ftp server. 
...
We will store the `base_url` where all Ensembl's species FASTA files are located and then the second part of the url is species-specific which you may have to replace. 

This base URL will not need to change should you use another species from Ensembl's release 95, but should you need a different release, you'll need to change `release-95` part correspondingly. 
...
The second parts of the URL are species specific and will require you to replace the pieces commented on below with your species' equivalent file path. 
...
Now we can follow a similar series of steps to find the file path for our species for the Gene Transfer Format (GTF) file using this address: 
...
Now this part of the URL will need to be replaced with your species' equivalent file path (if you are not using zebrafish). 
...
Side note, though not necessary, you can download these files and supply them to the next section as file paths to locations on your computer. 
In R, you can download files with `download.file()` function. 

## Create the `linkedTxome`

Now that we have located our files. 
Create a `linkedTxome` with the links to the FASTA files, GTF file, and salmon index specific to our species. 

The arguments, `source`, `organism`, release`, and `genome` are all single character strings that will need to be updated depending on the genome versions and files. 
...
For this example, we are using a [zebrafish cortisol experiment dataset](https://www.refine.bio/experiments/SRP073217/response-to-low-dose-cortisol-treatment-in-zebrafish-embryos-larvae) that we have already been quantified with salmon. 
