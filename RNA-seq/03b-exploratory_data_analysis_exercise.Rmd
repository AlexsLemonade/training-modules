...
We have prepared an RDS file that contains the output of `tximport` and a TSV file that contains metadata for the experiment.

```{r input-files}
tximport_file <- file.path("data", 
                           "tximport", 
                           "medulloblastoma",
                           "medulloblastoma_tximport.RDS")
metadata_file <- file.path("data",
                           "sample_metadata",
                           "medulloblastoma",
                           "SRP150101_metadata.tsv")
```

Use the chunk below to read in the tximport object and metadata; save them as `txi` and `metadata_df`, respectively.

```{r read-in-files, solution = TRUE}
# Read in the tximport file and save it as an object called txi

# Read in the TSV file that contains the metdata and save it as an object called
# metadata_df

```

Use the chunk below to look at the contents of `metadata_df`.

```{r examine-metadata, solution = TRUE}

```

Let's check if samples are in the same order.

```{r samples-same-order}
...
# The Run column of the metadata contains the run accession code from SRA
# We're checking this against the column order in txi
all.equal(metadata_df$Run, tximport_sample_names)
```

Let's look at the order of the samples in `txi`. 

```{r order-in-txi}
tximport_sample_names
```

The sample names are not in ascending order, which is what we saw in the notebook we went through together, so we can't just use `dplyr::arrange` without taking additional steps in this case.
Although these samples look like they're in descending order, we'll use the chunk below to show you a more general strategy for reordering metadata to match the expression data.
...
Let's use the chunk below to check the ordering again.

```{r samples-same-order-again, solution = TRUE}
# Now check if the samples are in the same order again

```

## DESeq2 

Now we're ready to create a `DESeqDataSet` from the tximport object.
Because we plan to use the `DESeqDataSet` to compare samples in a manner that is unbiased by the prior information about samples, we don't need to enter information about the experimental design to the `design` argument of `DESeqDataSetFromTximport()`; we can use `~ 1` instead.
...
Now make the same plot as you did above wiith `vsn::meanSdPlot()` using the transformed matrix.
...
The `intgroup` argument of `plotPCA()` takes the column name(s) from the metadata and will use that to color the points in the PCA plot.
