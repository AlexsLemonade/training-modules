...
Notice that `tx2gene_df` uses identifiers that don't include [Ensembl version information](https://useast.ensembl.org/Help/Faq?id=488) (e.g., don't end in `.1`).
We need to use the `ignoreTxVersion = TRUE` argument to `tximport` or else we'll get an error.
