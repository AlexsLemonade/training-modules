...
We'll add this as a new "toy" column in the sample data (`colData`) for `gene_summarized`.
...
If this batch information were real we would include it in the model stored in our DESeq2 object using the `design` argument. (`design = ~ tissue + batch`) and we would re-run the `DESeqDataSet()` and `vst()` steps we did above. 
