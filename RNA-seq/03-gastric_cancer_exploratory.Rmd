...
We'll put this in new "toy" column in `gene_summarized`.

```{r extract-sample, live = TRUE}
# Extract colData
sample_info <- colData(gene_summarized)
...
colData(gene_summarized) <- sample_info
```
                            
Now we need to rerun the `DESeq2` steps with this added toy batch column.

If this batch information were real we would include it in the model stored in our DESeq2 object using the `design` argument. (`design = ~ tissue + batch`). 

```{r rerun-deseq2, live = TRUE}
# Re-run DESeqDataSet() so `batch` column is included
ddset_batch <- DESeqDataSet(gene_summarized,
                            design = ~ tissue)
