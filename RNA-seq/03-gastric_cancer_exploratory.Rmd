...
```{r counts, live = TRUE}
# Extract the counts from gene_summarized
counts <- assay(gene_summarized, "counts")

# Let's look at the first few rows of the gene-level counts
head(counts)  
```

Alternatively, we could extract the TPM data from `gene_summarized`.

```{r abundance, live = TRUE}
...
We'll save the column names of the counts to a variable that tells us the order of the sample names in `counts`.
...
We'll reorder the sample metadata to match the order in `counts`
...
```{r all-equal}
all.equal(sample_names, sample_meta_df$srr_accession)
```

DESeq2 will want these counts to be rounded numbers. 
...
```{r ddset}
ddset <- DESeqDataSetFromMatrix(counts,
...
ddset_batch <- DESeqDataSetFromMatrix(counts,
                                      colData = sample_meta_df,
                                      design = ~ tissue)
# variance stabilizing transformation
