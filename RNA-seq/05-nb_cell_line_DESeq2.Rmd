...
Now we will use the information from `signif_genes` to filter `vst_data` to 
only have significant genes' data. 

```{r}
signif_vst_data <- vst_data %>%
  as.data.frame() %>%
  dplyr::filter(rownames(vst_data) %in% signif_genes$Gene)
```

Let's make the results into a heatmap using the `pheatmap` package. 

```{r}
pheatmap::pheatmap(signif_vst_data, show_rownames = FALSE)
```
