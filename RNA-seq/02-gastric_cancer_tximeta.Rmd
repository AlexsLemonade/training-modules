...
Using our `coldata` data frame we set up, we can run the `tximeta()` which will extract species specific information.

```{r read_tsv, live = TRUE}
txi_meta <- tximeta(coldata)
```

*There are extra steps needed if you are using any species other than human or mouse.
