...
All output files from `salmon quant` we'll use with `tximeta` are named 
`quant.sf`.
...
If we were to split up this character string by `/`, the second to last item is the sample name.
...
The first time you run `tximeta()` you will get a message about storing the data in a [cache](https://en.wikipedia.org/wiki/Cache_(computing) so that it will retrieve the data faster the next time.
We recommend you use the cache.
...
As shown in the diagram, we can use some of these functions to extract data from our `SummarizedExperiment` object. 
`rowData()` on our object shows all the gene information that `tximeta` set up!
