...
```{r metadata_files, solution = TRUE}
# name the metadata file path

# read in the metadata data.frame and call it metadata_df

```

```{r metadata-arrange}
# arrange metadata_df in the order of the sample `Run`
metadata_df <- metadata_df %>%
  dplyr::arrange(Run)
```

```{r check-order, solution = TRUE}
# check to make sure the metadata and txi object samples are in the same order using all.equal
...
# use DESeq2::DESeqDataSetFromTximport to create a ddset object
...
# Check directionality using a gene's data with a plot of your choice

# Set up a data.frame for plotting with ggplot2

# Set up ggplot2 plot basics

  # Make it a jitter plot but narrow the width of where the points will be

  # Add a theme to make it prettier

  # Make it on a log scale

  # Make the x axis lables more legible by turning them

  # The x label isn't particularly helpful

  # The y label should make it clear this is on a log scale
