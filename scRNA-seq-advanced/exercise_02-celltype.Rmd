...
In this exercise notebook we will work with the glioblastoma dataset in the first notebook, `01-read_filter_normalize_scRNA.Rmd`.
As a reminder, the sample was processed by 10X Genomics using a 3' RNA kit (v3.1), sequenced, and quantified with Cell Ranger 6.0. 
...
We will start with the output that we saved in `01-read_filter_normalize_scRNA.Rmd` that has already undergone filtering, normalization, dimension reduction, and clustering. 
...
We are going to be reading in the file containing the filtered and normalized glioblastoma data we saved in `01-read_filter_normalize_scRNA.Rmd`.
To do that we will need to define the file path, which will require two variables:

1. `data_dir`: This directory holds the normalized output from the intro instruction notebook, `data/glioblastoma/normalized`.
...
In the next chunk, use the `file.path()` function to define `data_dir` and `sce_file`.

```{r filepaths, solution = TRUE}
# data directory
...
Here we are examining a glioblastoma sample, so we expect it to include cell types typically found in the brain.
...
Conversely, we do not get to classify here is the presence of any T cells, because they are not found in our brain specific reference.
