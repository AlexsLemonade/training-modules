...
However, we have found that processing large `.h5` files is often _much_ less efficient in R, so so we prefer to start with the matrix files when possible.
...
![Single-cell roadmap preprocessing](diagrams/roadmap_single_preprocess.png)
...
(Though again, we do not recommend using the HDF5 file if you can avoid it, _especially_ for raw data.)
...
![Single-cell roadmap preprocessing](diagrams/roadmap_single_qc_norm.png)
...
![Single-cell roadmap preprocessing](diagrams/roadmap_single_dimension_reduction.png)
