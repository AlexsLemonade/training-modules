...
We can see from this summarythat this `SingleCellExperiment` (SCE) object contains 36,601 rows, which correspond to the features (genes) that were analyzed, and 734,492 columns, which correspond to the possible barcode tags that were used in the experiment.
...
In addition to the main `counts` matrix, which you can see listed as an `assay` in the `SingleCellExperiment` object summary above, the SCE object can contain a number of other tables and matrices, each stored in a "slot" with a particular format. 
...
There are also a couple of tables for metadata, and a slot to store reduced-dimension representations of the expression data.

We'll start with the `rowData` slot, which is a table of metadata for each feature in our object. 
For now, that for now contains the contents of the `features.tsv.gz` file that we discussed earlier.
...
Even if a droplet does not have a cell in it, there will often be spurious reads from RNA sequences that were present in the extracellular solution, whether from the original sample or cells that were disrupted during library preparation for single-cell analysis. 
...
As a consequence, there will be an over-abundance of mitochondrial reads.

Our first step then, is create a vector (list) of the mitochondrial genes that are present in our dataset.
...
We will use a cutoff of 200 unique genes as a minimum for the cells we want to keep. 
