...
Cell Ranger will also export these data in a single `HDF5` format file with a `.h5` extension, but we have found that using the separate matrix files as input seems to work more efficiently in R, so we prefer to start with those files.  
