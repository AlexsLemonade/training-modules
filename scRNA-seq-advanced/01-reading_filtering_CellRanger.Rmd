...
This representation is fast and fairly robust, but may not always provide the best graphical representation, in part because we will usually want to keep a fairly large number of components. 
...
This is a machine-learning-based method that is useful for performing dimensionality reductions suitable for visualization, but it is very important not to overinterpret the results it produces.
...
For the clustering below we will use Jaccard weighting and Louvain clustering, which correspond more closely to the default methods used by `Seurat`.
