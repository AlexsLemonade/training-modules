...
In this notebook, we will review basic processing for single-cell RNA-seq data, starting with the output from Cell Ranger, and proceeding through filtering, quality control, normalization, and dimension reduction. We will perform these tasks using tools from the [Bioconductor project](https://bioconductor.org), in particular the [`SingleCellExperiment` object](https://bioconductor.org/packages/release/bioc/html/SingleCellExperiment.html).
...
The sample was processed by  10X Genomics using a 3ʹ RNA kit (v3.1),  sequenced, and quantified with  Cell Ranger 6.0. 
Further details about the sample and processign can be found on the [10X website](https://www.10xgenomics.com/resources/datasets/2-k-sorted-cells-from-human-glioblastoma-multiforme-3-v-3-1-3-1-standard-6-0-0).
...
In addition to the main `counts` matrix, which you can see listed as an `assay` in the SCE object, there are two other tables of particular interest we will want to look at.

The first is the `rowData` table, which includes the contents of the `features.tsv.gz` file from earlier.
...
But barcodes with zero counts are not the only ones that will not correspond to droplets without cells in them!
Even if a droplet does not have a cell in it, there will often be spurious reads from RNA sequences that were present in the extracellular solution, whether from the original sample or cells that were disrupted during preparation for single-cell analysis. 
...
Here we will use the `emptyDropsCellRanger()` function to perform filtering that most closely matches the Cell Ranger implementation.


```{r calculate droplet stats}
# create a table of statistics using emptyDropsCellRanger
droplet_df <- DropletUtils::emptyDropsCellRanger(counts(raw_sce))
```
...
In addition to filtering out empty droplets, we also will want to filter out real cells that may have been damaged during library preparation.
These will often be characterized by a high proportion of mitochondrial transcripts.
This is because, if a cell ruptures, the cytoplasmic transcripts will leak out, but mitochondrial transcripts, protected by the mitochondrial membrane, will be more protected.
As a consequence, there is an over-abundance of mitochondrial reads.

Our first step then, is create a vector of the mitochondrial genes that are present in our dataset.
The mitochondrial file we defined during setup (`mito_file`) is a TSV file with additional annotation information for each gene such as the gene location and alternative names. 
All we need now is the `gene_id`, and only for the genes that are present in our SCE, so we will do some filtering with `dplyr` to pull out just a vector with those ids.
...
In this case, the line between kept and removed cells seems to correspond to a mitochondrial percentage of about 12.5%, but note that this will not always be constant: the cutoff point can vary for different numbers of unique genes.
...
(The data here are not _actually_ the log of the counts, since we also applied the scaling factors, but that is what we have.)
Let's take a look:
