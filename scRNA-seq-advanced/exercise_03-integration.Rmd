...
The data came from a number of different donors, and we have prepared a separate SingleCellExperiment (SCE) object for data from each donor.
...
What kinds of structure do you infer from these two plots? 
Are there per-donor effects?
Does it seem that the cell types are well-mixed across the different donors?
...
That should be a bit better, but it still may be hard to really see all of the individual points.

Alternatively, we can facet our plot by donor. 
To do this, use the `other_fields` argument with `plotReducedDim()` to add `donor` to the plot data.
Then use the `ggplot2` function `facet_wrap()` to split the points into separate panels by donor.

```{r plot faceted by donor, solution = TRUE}
# faceted UMAP plot
