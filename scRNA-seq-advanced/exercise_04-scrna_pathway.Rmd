...
Let's prepare our gene set list for input to `AUCell`.
We'll want to make a vector of Ensembl gene ids, and then save that to a list with an informative name we can refer to in our `AUCell` code.
In the chunk below, create a vector called `gene_ids` that contains all of our gene set's Ensembl gene ids.
Here are some things to keep in mind when preparing your gene set:

- You are only interested in the _tumor_ genes
- The `SingleCellExperiment` object uses Ensembl gene identifiers
- You'll want to remove any duplicate genes in the gene set

```{r create_marker_genes, solution = TRUE}
# create a vector named `gene_ids` with just the tumor genes

```

Now, we'll store it in a list to pass into `AUCell`.
Be sure to provide a name for the gene list, which should be "tumor_marker_genes".
Then assign the list to the variable named `ensg_tumor_markers`.

```{r format_marker_genes}
ensg_tumor_markers <- list("tumor_marker_genes" = gene_ids)
```
...
This object includes counts for all genes that were present in the index when quantifying gene expression, including genes that were not detected in any cells.
We don't want genes that are not found in our data set to impact our `AUCell` rankings, so before proceeding, remove any genes that aren't detected:

```{r filter_sce, solution = TRUE}
...
Next, we'll turn the data frame into a list named `collection_list` for use with `AUCell`.

```{r gene_set_collection}
collection_list <- split(collection_df$ensembl_gene, collection_df$gs_name)
```
...
You may need to change the names of the variables passed to the `exprMat` and `aucMaxRank` arguments below depending on what you used earlier!

```{r collection_auc}
# Change the variable names as needed
collection_cell_auc <- AUCell_run(
  exprMat = counts_matrix,
