...
- Demonstrate how AUC values can be used for assignment and plotting
...
AUCell calculates the area under the recovery curve, which "represents the proportion of expressed genes in the signature and their relative expression value compared to the other genes within the cell" ([Aibar _et al_. 2017.](https://doi.org/10.1038/nmeth.4463)).

We will use an snRNA-seq of a Ewing sarcoma sample from the [`SCPCP000015` project](https://scpca.alexslemonade.org/projects/SCPCP000015) on the Single-cell Pediatric Cancer Atlas Portal.
...
We will save the AUCell results as a table.
...
This number is probably too high, given the distribution of the number of genes detected by cell.
...
The scores or values we get out of AUCell are the area under a recovery curve and estimate the proportion of genes in the gene set are highly expressed (i.e., highly ranked).

Let's make two plots to get a better intuition about AUC values.
...
```{r write_auc_df, live = TRUE}
auc_df |>
  # Use shorter names to save to file
  dplyr::rename(zhang_auc = ewing_gene_set_names[1],
                riggi_auc = ewing_gene_set_names[2]) |>
  readr::write_tsv(file = output_file)
```

### Assignments

AUCell can assign cells as having an active gene set or not by picking a threshold.
...
auc_plotting_df <- auc_df |>
  ...
  dplyr::mutate(
    ...
    assigned = dplyr::case_when(
      # For Zhang gene set rows, set to TRUE when the barcode is in the assignment list
      gene_set == ewing_gene_set_names[1] & 
        barcodes %in% auc_assignments[[ewing_gene_set_names[1]]]$assignment ~ TRUE,
      # For Riggi gene set rows, set to TRUE when the barcode is in the 
      # assignment list
      gene_set == ewing_gene_set_names[2] & 
        barcodes %in% auc_assignments[[ewing_gene_set_names[2]]]$assignment ~ TRUE,
...
auc_plotting_df |>
  ...
  # Draw a vertical line showing the threshold for each gene set
