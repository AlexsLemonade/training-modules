...
We also see that there appear to by many more `Tumor_Mesoderm` cells in ERMS samples compared to ARMS samples.
Overall, these observations suggest that we may have less power to detect differentially expressed genes in `Tumor_Mesoderm` cells compared to `Tumor_Myoblast` cells.
As we proceed to the pseudo-bulking steps, we will check that we have sufficient sample sizes to perform our analysis.
...
To start exploring, let's pick out a couple genes from the top right and bottom left quadrants of the plot.
Based on the plot, it looks like we'll find the shared upregulated genes by looking for expression values that are greater than 8 for each gene, and we'll find the shared downregulated genes by looking for expression values that are less than about -10 for myoblasts and -7 for mesoderm cells.
...
Something you'll see in these results are some pretty "different" P-values between cell types (also note that the `NA` gene is a novel `lncRNA` which has no formally assigned gene symbol).
...
Let's do some quick visualization of some of these genes! 
Pick your favorite upregulated and/or downregulated gene from the list above and make some plots that spark joy. 
You can even feel free to adventure in a slightly different direction - do your own filtering to find genes that are in the _other_ quadrants (both cell types significant but in different directions) and make some plots for them as well!
...
# Use this chunk to make your own plots
