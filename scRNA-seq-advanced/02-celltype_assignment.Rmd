...
Since there is only one `altExp`, we don't need the second (name) argument (`"ADT"`) that we used above; the default behavior of `altExp()` is to just give us the first `altExp`, and that is all we need since there is only one.
...
The first way we will work on to identify the cell types of the individual cells in our sample is to work with the ADT markers that are part of this data set. 
These antibody markers were (hopefully) chosen for their relevance to the cell population we are working with.

The first marker we will look at is `CD3`, which is a protein complex that is found on the surface of T cells.
We can plot where this is found using the same `plotUMAP()` function as before. 

Note that the function is able to look for plotting data in fields found in the `colData` object (as we used it above), in the gene expression matrix (as we used it in the previous notebook), *AND* in `altExp` matrices!
So all we need to do to color by the ADT counts (as normalized into the `logcounts` matrix) is give the name of the tag that we want to plot in the `colour_by` argument.
...
When we have markers like these, with clear and established cell types, we might want to start our classification by working with these markers alone.
Such classification might be considered as analogous to a cell-sorter assay, where we would set up some rules to look at a few markers for each cell and use those to assign a cell type classification.

To do this, we will need to make some decisions, like what thresholds should we use to determine whether a cell is or is not expressing a particular marker. 
In general, if a marker is going to be useful for this type of assignment, we would like to see a bimodal plot for the marker in question that can be used to separate the population into two groups of cells based on the level of that marker.
One group of cells will have only a background level signal for each marker (due to non-specific binding or other factors), while the other group will have a much higher level of binding and higher counts.

To look at the expression levels of each of the marker genes we are interested in, we will pull out the expression values for the markers from the SCE object and do some data wrangling. 

The data we are interested is the normalized count data for the ADT tags, which is stored in the `logcounts` assay.
If you recall, this matrix is stored with the columns as cells and rows as markers, but we really want it the other way, so we will first transpose the data, then convert it to a data frame for our next steps.
Because the SCE object stores the main data matrices in a specialized format, we have to do one extra step convert it first to a "regular" R matrix or R won't know how to convert it to a data frame.
...
Right now the data is in a "wide" format, with each column a different tag. 
...
We can perform this conversion using the `tidyr::pivot_longer()` function.
Following conversion, we can filter to just the ADTs that we care about.
...
We can see 
There seem to be different baseline levels for each of the different tags, but each has a pretty nice bimodal distribution.
...
So while this technique is quite limited in its utility.
It might be good for some simple experiments, but it might not translate well to more complex data sets with multiple samples.
