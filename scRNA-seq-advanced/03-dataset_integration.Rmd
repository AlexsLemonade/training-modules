...
In this notebook, we'll perform integration of several scRNA-seq datasets from the [`ScPCA`](https://scpca.alexslemonade.org/), a database of uniformly-processed pediatric scRNA-seq data built and maintained by the Data Lab.
Specifically, we'll have a look at four libraries from the [`SCPCP000005` project](https://scpca.alexslemonade.org/projects/SCPCP000005), which contains rhabdomyosarcoma (RMS) data. 
We'll perform integration with two methods, **which we will either describe here or in the slides, or a little column a/little column b.**

- `fastMNN` 
- `harmony`

The four libraries we'll be looking were obtained from four different patients at recurrences and were sequenced as single-nuclei with 10Xv3 technology. 
These shared metadata properties hopefully make these datasets suitable for integration - it's important to bear in mind that not everything can/will integrate, and the success of the integration will usually depend on the given method used as well.
...
We have already pre-processed (e.g., filtered cells, normalized counts, and calculated PCA reduced dimensions) the four SCE objects we'll be integrating, and they are available in the `data/SCPCP000005-samples/processed/` directory. 
...
A different and more modular coding approach to reading in these files is to leverage the [`purrr`](https://purrr.tidyverse.org/) `tidyverse` package, which provides a convenient set of functions for _functional programming_.
...
Of particular interest is the [`purrr::map()`](https://purrr.tidyverse.org/reference/map.html) set of functions, which can be used to apply a given function to each element of a list or vector in one call.
...
Let's see it in action by quickly taking the square root of a list of values:
```{r map_example}
...
# The relative `map_dbl()` function always returns a vector of doubles (dbl)
purrr::map_dbl(squares, 
               sqrt)
```
...
```{r directories}
data_dir <- file.path("data", "SCPCP000005-samples")


# Define input directory containing processed SCE objects
input_dir <- file.path(data_dir, "processed")

# Define and create, as needed, the output directory where 
#  we will save the integrated object
output_dir <- file.path(data_dir, "integrated")
if (!(dir.exists(output_dir))) {
  dir.create(output_dir)
}

# Output file name for the integrated object
output_sce <- file.path(output_dir, "integrated_sce.rds")
```
...
We can use the handy `list.files()` function to help us find the names of all `.rds` files in the input directory.

```{r read input} 
# Find all files ending in `.rds` in the input directory
sce_paths <- list.files(input_dir, 
                        # find all files that ends in `.rds`
                        pattern = ".rds$", 
                        # return the relative paths to the files, not just the file names themselves
                        full.names = TRUE)
sce_paths
...
```{r add list names}
# This code will extract the portion of each `sce_paths` string that matches the pattern: 
#  "SCPCL" followed by a bunch of numbers
stringr::str_extract_all(sce_paths, "SCPCL\\d+")

# Assign the library names as the names for sce_list
names(sce_list) <- stringr::str_extract_all(sce_paths, "SCPCL\\d+")

sce_list
```

Something you'll notice is that these SCE objects all contain colData columns `celltype_fine` and `celltype_broad`.
These columns contain putative _cell type annotations_ as assigned in [Patel et al. 2022](https://doi.org/10.1016/j.devcel.2022.04.003). 
We can actually leverage these cell type annotations to explore how successful our integration is: After integration, we expect cell types from different samples to cluster together, rather than being separated by batches. 
...
Now that we have a defined list of processed SCE objects, we need to merge the objects into one overall SCE object which we can provide as input to integration methods.
A word of caution before we begin: **This merged SCE object is NOT an integrated SCE!** 
It still suffers from uncorrected batch effects, so it should not be used for any analyses besides input to integration itself.
...
- SCE objects should contain the same genes. In other words, the `rowData` slots should all have the same names
- SCE columns should match. In other words, the `colData` slots should all have the same names.
...
# Note the slightly different syntax here, used when the function is a 
#  stand-alone operation on the input
purrr::map(sce_list,
           ~names(colData(.)))
...
**REVIEWERS: Or we can just get rid of these columns altogether? Integration really only _needs_ the rownames.**
...
- The cell barcodes themselves (which are stored as `colData` rownames) can be modified to include library information
...
format_sce_list <- function(sce, library_name, shared_genes) {
  
  ###### Subset the SCE object to only contain the shared genes ######
  sce <- sce[shared_genes,]
  
  ###### Ensure gene-level stats can be identified ######
  # Q FOR REVIEWERS: WE COULD ALSO PURGE THE ROWDATA ENTIRELY EXCEPT FOR ROWNAMES?
  # Update the rowData names by prepending {library_name}, except for `gene_symbol`
  names(rowData(sce)) <- ifelse(
     # Is the value is gene_symbol?
    names(rowData(sce)) == "gene_symbol",      
    # If yes, keep gene_symbol
    "gene_symbol",       
    # If no, update the name by prepending the library name
    glue::glue("{library_name}-{names(rowData(sce))}")
  )
...
To run this function, we'll use the convenient `purrr::imap()` function, which _assumes_ that the second argument to the function we're calling are the list names:
...
We'll use the base R function `cbind`, but since the input is a list we'll have to use base R `do.call` to run `cbind` over the list:
...
To conveniently keep everything in one place, let's add these PCs information (with an informative name!) into our `merged_sce` object:
...
- Unlike `fastMNN`, `harmony` allows you to specify additional covariates (e.g., technology, patient, diagnosis, etc.)
  - This requires slightly more setup that what we will show below.
- Unlike `fastMNN`, `harmony` does not calculate corrected expression values nor does it return an SCE object.
Instead, it directly returns a matrix of corrected PCs.
...
# Let's store this directly into our `merged_sce` object, making sure to name it well!
...
```{r save integration}
readr::write_rds(merged_sce, output_sce)
```
