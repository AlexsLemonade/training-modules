...
# Just for now!!!!! This path will need to change BEFORE MERGING!
data_dir <- file.path("/shared/data/training-modules/scRNA-seq-advanced", "data", "rms", "integration")

# Output file name for the integrated object
output_sce <- file.path(data_dir, "integrated_sce.rds")
```
...
We can use the handy `list.files()` function to help us find the names of all relevant `.rds` files in the input directory.

```{r read input} 
# Find all files matching our pattern of `SCPCL<numbers>` in `.rds` in the input directory
sce_paths <- list.files(data_dir, 
                        # Regular expression matching the file name pattern `SCPCL<numbers>.rds`
                        pattern = "SCPCL\\d+.rds", 
...
```{r add list names}
# This code will extract the portion of each `sce_paths` string that matches
#  our filename pattern
