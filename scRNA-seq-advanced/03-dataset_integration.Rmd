...
3. SCE cell metadata columns should match: Each `colData` should have the same column names.
...
One way to ensure that barcodes remain unique even after merging is to actually modify the ids by _prepending_ the relevant library name. 
For example, we'll update the `CCTTTGGCACTACCCT` id in the `SCPCL000478` SCE to become: `SCPCL000478-CCTTTGGCACTACCCT`, thereby ensuring fully unique ids for all cells.
...
The final step in ensuring SCE compatibility is to make sure they all contain the same genes, found in the rownames of the `rowData`, which you can map back to gene names by simply asking my co-instructor Josh. 
