...
The particular libraries we'll integrate come from two rhabdomyosarcoma (RMS) patients, with two samples from two patients each, all sequenced with 10Xv3 technology.
...
Regardless of which integration tool is used, the `SingleCellExperiment` (SCE) objects first need to be reformatted and merged into a single (uncorrected!) SCE object that contains all cells from all SCES for all genes that the SCEs have in common.
This merged SCE, which should also contain explicit information about batch origins for each cell (here, the originating sample), can be used for integration to obtain a formally batch-corrected SCE object. 
...
integrated_sce_file <- file.path(output_dir, "rms_integrated_samples.rds")
```
...
That said, integration methods _do not actually use_ these cell type annotations.
...
1. We should be able to trace sample-specific information back to the originating sample, including...
    ...
    SCE column names represent unique cell ids, which are often barcodes.
    These cell ids (barcodes) are only guaranteed to be unique _within_ a library but may be repeated across libraries.
    So, after merging, it's technically possible that multiple cells will have the same barcode.
    This would be a problem for two reasons: First we wouldn't be able to identify the cell's originating sample.
    Second, it would literally cause an error in R, which does not allow duplicate column names.
    We need to be able to map each cell id back to its originating sample name and ensure that these cell id column names remain unique.
...
We must ensure that the column names of the SCE object are unique and directly add information to the SCE that indicates the different samples.
The best way to do this is simply to add a `colData` column with the sample information.
...
Because barcodes theoretically can be repeated across samples, we could end up in a situation with non-unique column names for the SCE object, which would make R complain!
...
For example, we'll update the `CCTTTGGCACTACCCT` id in the `SCPCL000478` SCE to become: `SCPCL000478-CCTTTGGCACTACCCT`, thereby ensuring fully unique ids for all cells across all samples.
...
To this end, we can take a similar approach to what we'll do for cell ids: We can change the sample-specific `rowData` column names by prepending the sample name.
...
Why can't we use the sample-specific PCA matrix? 
Part of PCA calculation itself involves scaling the raw data to center the mean.
When this is performed on each sample individually, all samples are separately centered, which leads to the data "overlapping" in space.
...
For input to integration, we'll want the PCA scaling to consider normalized gene expression values from all samples simultaneously, while also taking into consideration batch differences.
...
We'll use the [`batchelor::multiBatchPCA()`](https://rdrr.io/bioc/batchelor/man/multiBatchPCA.html) function accommodates the fact that we have separate batches, namely by ensuring that batches, which may have very different numbers of cells, contribute equally to the overall scaling.
...
# We can use indexing to see the PCA matrix calculated:
head(merged_pca[[1]])
```
...
This can serve as our "before" comparison before batch correction.
...
- The `corrected` reduced dimension represents the batch-corrected PCA that `fastMNN` calculated from the `logcounts` assay in the `merged_sce`. 
...
Recall from earlier, we conveniently have cell type annotations in our SCEs, so we can explore those here!
...
We see also that `SCPCL000481` seems to unique have a larger `Tumor_Myocyte` population, while all other samples have larger `Tumor_Mesoderm` populations. 
...
scater::plotReducedDim(merged_sce,
  ...
  ggtitle("UMAP after integration with fastMNN")
```
This UMAP is that the normal tissue cell types (mostly vascular endothelium, muscle cells, and monocytes) tend to cluster together and are generally far separated from the tumor cell types, which is an encouraging pattern!
...
This _potentially_ suggests that these Tumor cells were incorrectly classified and are actually not cancerous, which is a hypothesis one could further explore, but on the other hand RMS does share many features with muscle tissue.
...
One such method is , which (algorithm aside) has a couple practical differences from `fastMNN`:
...
Like `fastMNN`, `harmony` performs integration on an uncorrected a corrected (integrated) PCA matrix.
...
scater::plotReducedDim(merged_sce,
  ...
  guides(colour = guide_legend(override.aes = list(size = 3, alpha = 1))) 
```
This UMAP looks a little different from the `fastMNN` UMAP, in that `SCPCL000481` does not integrate as well with the other samples.
Again, this might be explained by the presence of a unique cell type population in this sample.
...
```
With the faceted view, we can now see that the small amounts of `Tumor_Mycote` from other samples are "pulled" towards those cells in `SCPCL000481`. 
