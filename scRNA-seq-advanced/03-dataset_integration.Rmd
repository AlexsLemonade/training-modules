...
Here is a schematic of what this wrangling will look like, **but a lot of the text that explains the technical steps happens in the next chunks and interweaved text so should this diagram be lower?**
...
Since this object is quite large (~855 MB!), we'll export it with some file compression to keep the output smaller.
This export will take a few minutes to run due to the compression step.
```{r export merged_sce}
...
_But_, we actually do not want to use these PCs specifically because they were calculated for each library individually.
Instead, we want to calculate new PCs from 
...
EXPAND ON THIS - we do this for individual libraries but that doesn't work here because we want to scale all gene expression in the same space, and take batch information into account. 

We can compare that UMAP to the UMAP calculated after performing integration.
...
multi_pca <- batchelor::multiBatchPCA(merged_sce,
                                      ...
                                      # Ensures that the returned object is a single 
                                      #  matrix that contains all samples
                                      preserve.single = TRUE)


# print MAYBE? I should look at this object literally at least once.
multi_pca
...
Explain here about scaling and on top of each other != integration but an "artifact" of doing PCA separately on each library. 
We see when libraries are merged, the batches are separated in space. Yes.
