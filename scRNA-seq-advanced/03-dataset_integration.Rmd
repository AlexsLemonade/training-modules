...
These shared metadata properties hopefully make these datasets suitable for integration - it's important to bear in mind that not all datasets can/will integrate.

We'll be performing integration with two different tools, `fastmNN` and `harmony`. 
One reason we're using two approaches is to compare their performance; as we might see (foreshadowing!), different methods may perform differently!
...
Once we have PCs, we can proceed to calculate UMAP.
```{r}
...
```{r save integration}
# I'm getting disk space errors? but the file is being written? weird stuff.
#readr::write_rds(merged_sce, integrated_sce_file)
```
