...
Of particular interest is the [`purrr::map()`](https://purrr.tidyverse.org/reference/map.html) family of functions, which can be used to run a given function on each element of a list in one call.
The general syntax for `purrr::map()` and friends is:

```
# Syntax for using the map function:
purrr::map(input list, 
           function to apply to each item in the input list, 
...
In fact, this is conceptually the same as base R's `lapply` function, and the `purrr::map()` family of functions provide more robust and reliable alternatives to the base R `apply` functions.
