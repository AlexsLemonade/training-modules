...
Recall, these are the library files:

```{r dir input2}
dir(input_dir)
```

We'll first need to define a vector of the library file paths to read in.
We'll start by creating a vector of library names themselves and then formatting them into the correct paths.
This way (foreshadowing!) we also have a stand-alone vector of just library names, which will come in handy!

```{r read input} 
# Vector of all the library names to read in:
...
1. We should be able to trace library-specific information back to the originating library, including...
  ...
  These cell ids (barcodes) are only guaranteed to be unique _within_ a library but may be repeated across libraries, so after merging, it's technically possible that multiple cells have the same barcode.
  This would be a problem for two reasons: First we wouldn't be able to identify the cell's originating library.
  Second, it would literally cause an error in R, which does not allow duplicate coumn names.
  We need to ensure that these cell id column names remain unique for two reasons - to be able to identify cells, and to avoid R errors cause by duplicate column names), 
...
3. SCE cell metadata columns should match: Each SCE object's `colData` should have the same column names.
