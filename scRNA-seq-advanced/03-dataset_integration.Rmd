...
1. We should be able to trace sample-specific information back to the originating sample, including...
    ...
    These cell ids (barcodes) are only guaranteed to be unique _within_ a sample but may be repeated across samples.
    ...
    First, we wouldn't be able to identify the cell's originating sample.
...
We also must ensure that the column names of the SCE object are unique, even when combined with other SCE objects.
The current column names are cell ids (barcodes) that are only guaranteed to be unique _within_ a sample but may be repeated across samples.
This would be a problem for two reasons: 
First we wouldn't be able to identify the cell's originating sample.
Second, it would literally cause an error in R, which does not allow duplicate column names.
