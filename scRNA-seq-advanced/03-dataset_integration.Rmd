...
Specifically, we'll have a look at four samples (where each sample has a single associated library) from the [`SCPCP000005` project](https://scpca.alexslemonade.org/projects/SCPCP000005; [Patel _et al._ (2022)](https://doi.org/10.1016/j.devcel.2022.04.003)), an investigation of pediatric solid tumors led by the [Dyer](https://www.stjude.org/research/labs/dyer-lab.html) and [Chen](https://www.stjude.org/research/labs/chen-lab-taosheng.html) labs at St. Jude Children's Research Hospital.
The particular samples we'll integrate come from two rhabdomyosarcoma (RMS) patients, with two samples from two patients each, all sequenced with 10Xv3 technology.
...
This merged SCE, which should also contain explicit information about batch origins for each cell (here, the originating sample), can be provided to the integration method to obtain a formally batch-corrected SCE object. 
...
Thaat said, integration methods _do not actually use_ these cell type annotations.
...
1. We should be able to trace sample-specific information back to the originating sample, including...
    ...
    We need to be able to map each cell id back to its originating library and ensure that these cell id column names remain unique.
