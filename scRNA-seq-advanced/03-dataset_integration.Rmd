...
But, the output from this code will be a list, since that's what `purrr::map()` always returns.
(We could convert `sce_paths` to a list first with `as.list(sce_paths)`, but for this example, the code will work just the same!)
