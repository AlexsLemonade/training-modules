...
Integration corrects for batch effects that arise from different library preparations, genetic backgrounds, and other sample-specific factors, so that samples can be jointly analyzed. 
...
Regardless of which integration tool is used, the SCE objects to be integrated need to be reformatted and merged into a single (uncorrected!) SCE object that contains all cells from all SCES for all genes that the SCEs have in common.
...
The files are interested in are named according to their `ScPCA` library IDs:
...
We could also use a `for` loop, which the approach that many programming languages would lean toward.
...
3. SCE cell metadata columns should match: Each SCE object should have the same column names.
...
```{r barcodes}
# Look at the column names for the `SCPCL000478` library, for example
colnames(sce_list$SCPCL000478) %>%
  # Only print out the first 6 for convenience
  head()
```

Because barcodes theoretically can be repeated across libraries, we could end up in a situation with non-unique rownames for the `colData`, which would make R complain!
...
The final step in ensuring SCE compatibility is to make sure they all contain the same genes, which are stored as the SCE object's row names (these names are also found the `rowData` slot's row names). 
Here, those gene ids are unique Ensembl gene ids.
We'll note that while we use to recommend the `biomartr` package for querying Ensembl gene information, we now favor the approach of just asking Josh.
...
The main reason for using these reduced dimenions is efficiency.
...
We'll use the [`batchelor::multiBatchPCA()`](https://rdrr.io/bioc/batchelor/man/multiBatchPCA.html) function which can accommodate the fact that we have separate batches.
Specifically, this function performs scaling  function performs in our SCE such that the scaling corrects for the different number of cells across batches.
...
Now that we've put all of the samples from our `merged_sce` object in the same UMAP space, we see that the libraries are separated in space, which more reasonably reflects that this data is _not yet batch-corrected_.
Indeed, the primary "clustering" (not technical clustering, just what we see visually!) in these plots are the samples.
...
One way we can see all the points a bit better is to facet the plot by sample:
