...
We have already pre-processed (e.g., filtered cells, normalized counts, and calculated PCA reduced dimensions) the four SCE objects we'll be integrating, and they are available in the `data/rms/integration/` directory. 
...
Of particular interest is the [`purrr::map()`](https://purrr.tidyverse.org/reference/map.html) family of functions, which can be used to run a given function to each element of a list or vector in one call.
...
# The `map()` relative `map_dbl()` function always returns a vector of doubles (dbl)
...
# This full path is just for now until data is linked, which will happen BEFORE MERGING THIS!!!!
...
sce_paths <- list.files(data_dir, 
                        # This regular expression forms the pattern: "SCPCL<numbers>rds"
                        pattern = "SCPCL\\d+.rds", 
...
When calculating PCs on this merged object, we need to use a slightly different approach that respects the batch differences within. 
...
reducedDim(merged_sce, "merged_PCA") <- multi_pca@listData[[1]]
...
scater::plotReducedDim(merged_sce,
                       ...
                       # color by broad celltypes
                       colour_by = "celltype_broad",
                       # Let's also shape by the batches:
                       shape_by = "sample",
                       # Some styling to help us see the points:
                       point_size = 0.5,
                       point_alpha = 0.2) +
