...
If we were to keep each cell's counts separate, treating them as replicate samples, we would end up with inflated statistics. 
...
In particular, we can use principal component analysis (PCA) of our pseudo-bulked dataset as an initial exploration of our hypothesis that ARMS and ERMS samples have different gene expression patterns.
...
```{r add gene symbol}
# convert shrunken results to a tibble and add Ensembl id as a column
...
scater::plotExpression(tumor_sce,
                       features = genes_to_plot, # a vector of genes to plot
                       x = "diagnosis_group", 
                       colour_by = "diagnosis_group",
