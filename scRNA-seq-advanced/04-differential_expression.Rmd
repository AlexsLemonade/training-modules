...
The reason for this is two-fold - many of the DE models will expect uncorrected counts because they will account for between sample variation, and we want to ensure we are preserving variation that is present so as not to artificially inflate differences between populations.
...
`DESeq2` expects raw counts as input so we will be using data found in the `counts` assay.
...
Genes that are homogeneous across a sample are more likely to be identified as differentially expressed, while genes with high intra-sample heterogeneity will have a weaker signal.
...
Next we will create a pseudo-bulked matrix, with only 2 columns, 1 for group A and 1 for group B.
To do this we will use the `DelayedArray::colsum()` function that allows us to sum the counts in all columns.
...
pseudobulked_counts <- DelayedArray::colsum(counts_mtx, 
                                            groups)
```
...
We will use the [`aggregateAcrossCells()`function from the `scuttle` package](https://rdrr.io/github/LTLA/scuttle/man/aggregateAcrossCells.html) to pseudo-bulk our dataset.
...
This is because these values were initially calculated on a per cell level (we did this using `scuttle::addPerCellQCMetrics()`), but we no longer have a single column per cell, each column represents a group of cells, in this case cells of a specified cell type from an individual sample.
Therefore, the numbers that we calculated on a per cell level are no longer applicable to this pseudo-bulked SCE object.
...
If there is variation in their gene expression patterns we would expect samples from each RMS subtype to separate in PC space, likely indicating presence of differentially expressed genes.
...
As a reminder, this is NOT required for running `DESeq2` analysis, we are using it to visualize our data prior to DE analysis.
