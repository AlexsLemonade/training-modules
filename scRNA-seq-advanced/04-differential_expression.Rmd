...
Unlike in bulk RNA-seq analysis, single-cell RNA-seq analysis allows us to identify and annotate cell types or subpopulations of cells present in each of our samples.
...
RMS tumors, regardless of subtype, are made up of cells typically associated with development of skeletal muscle: mesodermal cells, myoblasts, and myocytes ([Sebire and Malone 2003](https://doi.org/10.1136/jcp.56.6.412)).
...
Because we are going to be doing DE analysis between ARMS and ERMS samples, let's start by first looking at our integrated dataset and labeling our cells by which RMS subtype they are from.
...
Fortunately we also have the sample metadata table, which contains information about each of the samples present in the dataset, such as RMS subtype, age, sex, etc. 
...
In the chunk below we will color our cells by RMS subtype.
...
One reason is that tumor data can be heterogenous and every tumor is unique.
...
To confirm our hypothesis that cells are grouping together both by cell type and by RMS subtype we can create a plot that incorporates both pieces of metadata.
