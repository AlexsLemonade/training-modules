...
coldata_df <- colData(integrated_sce) %>%
  ...
  dplyr::mutate(cell_id = paste(sample, barcode, sep = "-"),
...
These plots show that ARMS and ERMS share a lot of the same cell types, but perhaps there are differences in the cells in each subdiagnoses.
...
```{r}
# we are left with 3 from each subdiagnosis 
sample_metadata %>%
...
scater::plotExpression(tumor_sce,
                       ...
                       point_size = 0.1) +
  facet_wrap(Feature ~ celltype_broad, ncol = 4) + 
