...
The ARMS subdiagnosis is characterized by a fusion gene with the transcription factor, `FOXO1`, leading to a distinct changes in downstream gene and protein expression, likely accounting for the global gene expression changes seen between samples from these two subdiagnoses[REF].
We would also expect that the same cell types may be present in both, but may have different gene expression patterns causing the separation seen in the UMAP.
