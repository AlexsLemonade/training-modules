...
Additionally, cells found in ARMS tumors tend to have an increased mutational burden and cells are in a more differentiated state in comparison to ERMS tumor cells ([Shern _et al._ 2014](https://doi.org/10.1158/2159-8290.CD-13-0639); [Stewart _et al._ 2018](https://doi.org/10.1016/j.ccell.2018.07.012)).
...
In [Patel _et al._ (2022)](https://doi.org/10.1016/j.devcel.2022.04.003), they tested the hypothesis that cell types have distinct gene expression patterns in ARMS vs. ERMS samples.
...
It's important to note that DE analysis can be applied to any subpopulation of interest that is shared across samples and does not need to be explicitly defined by a cell type.
...
coldata_df <- colData(integrated_sce) |>
  ...
  dplyr::mutate(cell_id = glue::glue("{sample}-{barcode}"), # combination of barcode and sample
...
Now we are ready to start preparing for our DE analysis, where we will compare the gene expression of all cells annotated as `Tumor_Myoblasts` in 3 ARMS samples to 3 ERMS samples.
...
For the remainder of the DE analysis we will be using data found in the integrated object that has been uncorrected. 
...
Psuedo-bulking will create a new SCE object with new dimensions that contain only the pseudo-bulked counts for all genes. 
...
```{r pseudobulk matrix}
# create pseudo-bulked matrix, summing the counts for each gene across all cells in the specified group
...
As mentioned previously, we want to are specifically interested in the `Tumor_Myoblast` cell type.
...
Now we will use the `DESeq2` package to perform DE analysis on our pseudo-bulked SCE object.
...
For our experiment we are comparing gene expression between different RMS subtypes, stored in the `diagnosis_group` column of the `colData` in the pseudo-bulked SCE.
...
This is because we took the unnormalized counts from the original SCE to create the new pseudo-bulked counts which then become the input to create the `DESeq2` object.
...
Therefore we don't have to do any normalization on our own and we can let `DESeq2` do all the work for us.
...
If there is variation in their gene expression patterns we would expect samples from each RMS subtype to separate in PC space.
...
If you look at our `shrink_results` object, we see that the genes are labeled with the ensembl gene identifiers, as those were the rownames of the pseudo-bulked SCE we used as input to build our `DESeq2` object.
...
Now that we have identified a set of genes that are DE in the tumor myoblasts between ARMS and ERMS subtypes, lets actually take a look at them and see if we can make some informative plots.
...
We can create UMAP plots as we did previously but instead of labeling each cell with metadata we can color each of the cells by the level of gene expression of a specified gene. 
...
However, we might be interested to see the expression of genes that are differentially expressed in tumor myoblasts in other cell types present in our samples.
...
Next we will look at a few DE genes that we identified, one upregulated gene and one downregulated gene, and compare their expression in myoblasts to other cell types in ARMS and ERMS samples.
...
We can continue using our `facet_grid()` functionality and show each cell type in its own column of the plot grid.
...
scater::plotExpression(tumor_sce,
                       features = genes_to_plot, # provide a vector of genes to plot
                       x = "diagnosis_group", 
                       colour_by = "diagnosis_group",
                       other_fields = "celltype_broad",
                       point_size = 0.1) +
  facet_grid(cols = vars(celltype_broad), # each celltype is its own column
             rows = vars(Feature)) + # each feature is its own row
  theme(strip.text = element_text(size = 7)) + # change the font size of the facet labels
  guides(colour = guide_legend(override.aes = list(size = 3, alpha = 1))) # change the size of the legend colors
```
