...
We will read in a `SingleCellExperiment` object that contains both the uncorrected (merged but not integrated) and corrected (post-integration) gene expression data for all 10 samples.
...
Prior to integration all 10 samples have gone through the same filtering, normalization, and dimensionality reduction.
...
The final `SingleCellExperiment`(SCE) object, stored in `data/rms/integrated/rms_all_sce.rds`.
...
Here we should have data from both the uncorrected and the corrected, integrated data for both the gene expression data and the reduced dimensionality results.
...
The `counts` and `logcounts` assay correspond to the uncorrected gene expression data that has just been merged and has NOT been integrated.
...
Both the `fastmnn_PCA` and `fastmnn_UMAP` obtain the dimensionality reduction calculated post-correction and integration.
...
**Note: Be sure to use the batch-corrected dimensionality reduction results for visualizing datasets containing multiple libraries**
...
In the below chunk we will color our cells by subdiagnosis.
...
Not all samples contain tumor cells that were further classified and samples will either have all `Tumor` or a mix of `Tumor_Mesoderm`, `Tumor_Myoblast`, and `Tumor_Myocyte`._
...
We can take advantage of another `ggplot` tool, the `facet_wrap()` function to look at two (or more, but we'll start with two) variables in the `colData` at once - the cell type and the subdiagnosis.
...
Similar to the UMAP, this plot shows that ARMS and ERMS share a lot of the same cell types, although it is likely that these cell types may have distinct gene expression patterns defined by the associated subdiagnosis.
...
We would want to remove any samples that do not contain our population of interest as they would have no counts to contribute to the differential expression analysis.
...
If we were to look at our stacked boxplot again we would see that this cell type is shared among all 6 of our selected samples. 
...
Now we are ready to start preparing for our differential expression analysis.
Here we are specifically testing the hypothesis that tumor myoblasts in ARMS samples express different genes than tumor myoblasts from ERMS samples.
To test this hypothesis, we will compare the gene expression of all cells annotated as `Tumor_Myoblasts` in 3 ARMS samples to 3 ERMS samples.

Throughout the notebook we have been working with an integrated dataset that contains corrected gene expression data and a corrected UMAP.
However, we do not want to use corrected gene expression values for differential expression, we want to use the original raw counts that are present in the `counts` assay of the `SingleCellExperiment` object. 
It is advised to only used the corrected values for any analyses being performed at the cell level, e.g. dimensionality reduction.
...
See the [OSCA chapter on Using the corrected values](http://bioconductor.org/books/3.15/OSCA.multisample/using-corrected-values.html#using-corrected-values) for more insight.

For the remainder of the differential expression analysis we will be using data found in the integrated object that has been uncorrected. 
As a reminder, the uncorrected gene expression data, in the `counts` and `logcounts` assays, correspond to data that has been merged (the first step we walked through prior to integration) but has not been corrected.
...
Pseudo-bulking creates a new counts matrix that contains the total counts across all cells of a given label (e.g. cell type) for each sample ([Tung _et al.,_ 2017](https://doi.org/10.1038/srep39921)). 
If each gene had counts from every cell, then each cell would be counted as its own replicate and we would inflate our p-values.
By pseudo-bulking first, each gene will now have one value for each sample and we can utilize the same methods we would use if we were performing bulk RNA-seq.

Pseudo-bulking is implemented prior to differential expression analysis on single-cell data for the following reasons: 

- Produces larger counts, which allows us to use standard normalization and differential expression methods used by bulk RNA-seq. 
...
When we pseudo-bulk we want to produce a new counts matrix that has all genes and condenses all cells to include one group for each combination of cell type and sample.
Psuedo-bulking will create a new `SingleCellExperiment` (SCE) object with new dimensions that contains only the pseudo-bulked counts for all genes. 
We can then subset this SCE to just contain our cell type of interest (tumor myoblasts) for input to differential expression analysis. 
...
```{r pseudobulk sce}
# first subset the coldata to only have the columns we care about in pseudo-bulking 
pseudobulked_groups <- colData(rms_sce)[, c("celltype_broad", "sample")]

# create a new SCE object that contains the pseudo-bulked counts across the provided groups 
pseudobulked_sce <- scuttle::aggregateAcrossCells(rms_sce, 
                                                  id = pseudobulked_groups)

# column names aren't automatically added to the pseudo-bulked sce, so let's add them in 
...
This new object only has one assay, the `counts` assay. 

```{r counts}
# print out the counts matrix
counts(pseudobulked_sce)[1:10, 1:10]
```

This `counts` assay should look a little different then the assays we've seen previously.
The gene expression counts should be higher and you should see less 0's because we've now summed counts across all myoblasts in each sample to create this matrix.
...
head(colData(pseudobulked_sce))
```

You should see a new column that wasn't present previously, the `ncells` column.
...
Before we proceed we will actually want to filter out any columns that have a low number of cells.
A low number of cells will translate to a low number of counts cause issues with statistical approximations made during differential expression analysis.
...
We can then take a look and see how many cell types we removed, if any.
...
The last step we want to do to prepare our dataset for DE is to subset the pseudo-bulked `SingleCellExperiment` object to contain only the cell type that we are interested in comparing across the two subdiagnoses.
As mentioned previously, we want to compare the gene expression of the `Tumor_Myoblast` type across subdiagnoses.
...
```{r summary table}
# group the coldata by diagnosis group and celltype and print out the counts
head(colData(tumor_myoblast_sce)) %>%
  as.data.frame() %>%
  dplyr::count(diagnosis_group, celltype_broad)
```

### Perform differential expression with DESeq2

Now we will use the `DESeq2` package to perform differential expression analysis on our pseudo-bulked dataset.
...
First we will create the `DESeq2` object.
To create the object we will need the unnormalized counts matrix, the metadata associated with the samples that will help tell `DESeq2` which sample belongs to which group, and the design formula. 
The design formula is used to indicate which columns of the metadata need to be considered in the comparison.
For our experiment we are comparing gene expression between different diagnosis groups.
The diagnosis group can be found in the `diagnosis_group` column of the `colData` in the pseudo-bulked SCE object.

```{r deseq object}
# extract the metadata for each sample from sce as data.frame
...
deseq_object <- DESeq2::DESeqDataSetFromMatrix(counts(tumor_myoblast_sce),
                                               ...
                                               colData = coldata_df,
                                               # name of column to use for grouping samples 
                                               design = ~ diagnosis_group)
```

Before we can compare gene expression we will need to normalize the counts data.
If you look at the pseudo-bulked SCE object you should have only seen one assay, the `counts` assay.
This is because we took the unnormalized counts to create the new pseudo-bulked counts to create our `DESeq2` object.
We don't want to sum the normalized counts because those have each been normalized within their sample and do not account for differences between samples.
...
Therefore we don't have to do any normalization on our own. 

However, we want to take a look at our data before we run the DE analysis.
In particular, we can use principal component analysis (PCA) of our pseudo-bulked dataset as an initial test of our hypothesis that ARMS and ERMS samples have different gene expression patterns.
If there is variation in their gene expression patterns we would expect samples from each subdiagnosis to separate in PC space.
...
As expected we see that samples group together based on subdiagnosis and are separated along the PC1 axis, the PC contributing the highest amount of variation.
...
If you look at our `shrink_results`, we see that the genes are indicated with the ensembl gene identifiers, as those were the rownames of the pseudo-bulked SCE we used as input to build our `DESeq2` object.
...
```{r add gene symbol}
# convert shrunken results to a data.frame and add ensembl id as a column
...
We can save the new `data.frame` that we have created with both the ensembl identifiers and the gene symbols and the `DESeq2` results as a tab separated (`tsv`) file.
...
Here we will want to use the adjusted p-value, found in the `padj` column of the results, as this p-value accounts for multiple test correction.
