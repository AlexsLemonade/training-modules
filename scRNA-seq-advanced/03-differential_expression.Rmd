...
To do this we will use the `S4Arrays::colsum()` function, which allows us to sum the counts for each row across groups of columns.
(Interestingly enough, there is a base R function called `rowsum()`, but not an equivalent for columns.)
...
pb_counts <- S4Arrays::colsum(counts_mtx, 
                              groups)
pb_counts  
```
...
We can also return to the SCE object that we used to create our pseudo-bulked SCE and look at gene expression of some of the significant genes. 
