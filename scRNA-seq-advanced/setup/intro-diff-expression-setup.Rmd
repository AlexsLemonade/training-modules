...
If we want to ask that question, then we need to have replicates, e.g. more than one sample for each type of patient, which we do! 
...
Pseudo-bulking the counts results in a new counts matrix that has been produced from summing the counts together for all cells with the same combination of label (e.g. cell type) and sample. 
...
- Produces larger counts which are more amenable to standard normalizationa and differential expression methods used by bulk RNA-sequencing. 
- Collapses gene expression counts by sample, reflecting that each sample is a replicate rather than each cell. 
- Masks variance within a sample and only accounts for if a gene is uniformly high or low across samples. 
...
genes_filter <- edgeR::filterByExpr(tumor_myoblast_sce,
                                     group = tumor_myoblast_sce$subdiagnosis)
tumor_myoblast_sce <- tumor_myoblast_sce[genes_filter,]
...
Estimate dispersions using both the QL dispersion and the negative binomial(NB) dispersions. 
...
Test for differences using the `glmQLFTest` where DEGs are defined as those with non-zero LFC and a FDR of < 5%.
...
Look at the correlation of gene expression for all pairwise combinations of samples in the dataset and perform heirarchical clustering.
Helps reveal which samples are more similar to each other so we would expect to see separation between the two diagnosis groups. 
...
We also see that theres an extra subset of genes (with lower abs(log fold changes)) that are identified as differentially expressed in DESeq only. 
...
scater::plotReducedDim(myoblast_combined_sce,
                       ...
                       point_size= 0.5,
                       point_alpha = 0.4) +
  facet_wrap(~ myoblast_combined_sce$batch + myoblast_combined_sce$subdiagnosis, nrow = 3)
```
...
- The RMS dataset seems like a good dataset we can use for integration
