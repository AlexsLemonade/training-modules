...
# Dataset link: https://www.10xgenomics.com/resources/datasets/10-k-cells-from-a-malt-tumor-gene-expression-and-cell-surface-protein-3-standard-3-0-0
...
These are all human surface proteins except `IgG1`, which is a mouse isotype that was used as a control, which we can determine via the Excel sheet "Download the excel file for a full list of clones and barcodes" from the kit page: https://www.biolegend.com/en-us/products/totalseq-b-human-universal-cocktail-v1dot0-20960?GroupID=GROUP28

If there's no contamination, we expect ~0 counts for this protein since the data is human:
...
First we'll build the `miQC` model and visualize it, including how it suggests to filter the cells.
```{r}
miqc_model <- miQC::mixtureModel(filtered_sce) 

# Show some miQC plots -
...
Now, let's filter based on `miQC`:
```{r}
# Apply miQC filtering
...
We will focus on only removing cells with `cleanTagCounts` which will flag cells for removal if there is no detectable ambient concentration, based on an ambient scaling factor that the function calculates.
Note that "cells with unusually high values" also get flagged for removal since the they "are assumed to be affected by protein aggregates." 
...
# Keep only cells where discard is TRUE
...
```markdown
/home/ccdl/stephanie/.cache/R/ExperimentHub 
  does not exist, create directory? (yes/no):
```
Everyone needs to type **yes** since there is no default.
If they hit enter, the prompt will keep prompting.  

```{r}
ref_data <- celldex::HumanPrimaryCellAtlasData(ensembl = TRUE) # pick one for now to start

# Note that this variable has an uppercase R - watch for case typos!
...
norm_sce$celltype_predictions <- singleR_predictions$pruned.labels

# **heavily** biased towards monocytes and T-cells
celltype_counts <- singleR_predictions %>%
  as.data.frame() %>%
...
```{r}
SingleR::plotScoreHeatmap(singleR_predictions)
```
...
It might be helpful to hone in on the most prevalent ADT for each cell:
