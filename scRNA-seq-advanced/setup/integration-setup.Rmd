...
params:
  setup_only: FALSE
  input_sce_path: "`r file.path('SCPCP000005-samples')`"
  output_sce_path: "`r file.path('..', 'data', 'SCPCP000005-samples')`"
---
...
We use four different samples from this project that have the same technology (single-nucleus, 10Xv3), and these unproccessed samples are expected to be saved in the directory specified in the `input_sce_path` parameter.
...
```{r}
celltypes_file <- file.path(params$input_sce_path, "celltypes.tsv")
...
We can go ahead and exit this notebook if it's being run for data setup only.
```{r}
if (params$setup_only == TRUE) {
  knitr::knit_exit()
}
```
