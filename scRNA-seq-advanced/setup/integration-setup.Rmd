...
We use four different samples from this project that have the same technology (single-nucleus, 10Xv3), and these are expected to be saved in the directory `SCPCP000005-samples/`.

- `SCPCS000252` (`SCPCL000478`)
- `SCPCS000253` (`SCPCL000479`)
- `SCPCS000254` (`SCPCL000480`)
- `SCPCS000255` (`SCPCL000481`)
...
# We'll export processes SCEs here:
output_dir <- file.path("..", "data", "SCPCP00005")
if (!(dir.exists(output_dir))) {
  dir.create(output_dir, recursive = TRUE)
}

# Make this a list of paths:
sce_paths <- list.files(data_dir, pattern = "_filtered.rds", full.names = TRUE)
...
```{r}
celltypes_file <- file.path(data_dir, "celltypes.tsv")

celltypes_df <- readr::read_tsv(celltypes_file)
celltypes_df
...
celltypes_df <- celltypes_df %>%
  dplyr::mutate(celltype_broad = dplyr::case_when(
    stringr::str_starts(celltype, "Tumor_Myoblast") ~ "Tumor_Myoblast",
    stringr::str_starts(celltype, "Tumor_Myocyte") ~ "Tumor_Myocyte",
    TRUE ~ celltype
  )) %>%
...
add_celltype_info <- function(sce, sce_name, celltypes_df) {
  ...
  # Join cell type information in
  colData(sce) <- celltypes_df %>%
    dplyr::filter(library == sce_name) %>%
    dplyr::select(-library) %>%
    # Use a right_join since there are more cells in the SCE than there are in
    #  the celltypes
    dplyr::right_join( as.data.frame(colData(sce)), by = "barcode" ) %>%
  S4Vectors::DataFrame()
  
  # Don't lose the rownames!
...
# Before
before_umap <- scater::plotReducedDim(combined_sce,
                       dimred = "combined_UMAP",
  ...
  # larger legend key
  ggplot2::guides(colour = ggplot2::guide_legend(override.aes = list(size = 3, alpha = 0.8)))


# fastmnn
fastmnn_umap <- scater::plotReducedDim(combined_sce,
                       dimred = "fastmnn_UMAP",
                       colour_by = "celltype_broad",
                       # Some styling to help us see the points:
                       point_size = 0.5,
...
# harmony
harmony_umap <- scater::plotReducedDim(combined_sce,
                       dimred = "harmony_UMAP",
                       colour_by = "celltype_broad",
                       # Some styling to help us see the points:
                       point_size = 0.5,
...
shared_legend <- cowplot::get_legend(before_umap)

umap_row <- cowplot::plot_grid(
  before_umap + ggplot2::theme(legend.position = "none"),
  fastmnn_umap, 
  harmony_umap,
  nrow = 1
)
  
cowplot::plot_grid(umap_row, shared_legend, nrow = 2, rel_heights = c(1, 0.2))
```
