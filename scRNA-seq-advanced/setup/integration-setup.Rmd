...
To get the raw data:
```{sh eval = FALSE}
# Unclear to me what the link is to curl/wget these....?
```
...
- a bonus key way is that `harmony` is on CRAN but `batchelor` is on Bioc
...
integrated_sce_harmony <- harmony::HarmonyMatrix(
      # PCs
...
head(integrated_sce_harmony)

# Let's store this directly into our `combined_sce` object, making sure to name it well!
reducedDim(combined_sce, "harmony_PCA") <- integrated_sce_harmony
