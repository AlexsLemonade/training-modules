...
A quick aside!
When we ran `alevinQC` on this data in the last notebook, we saw that `salmon` had identified a "whitelist" of barcodes, i.e. barcodes that passed its quality control standards.
Here, we are actually going to read in all barcodes, regardless of whether they are on the whitelist, so that we can explore different filtering thresholds ourselves (`salmon` can be very stringent when filtering out barcodes).
But, if you wanted to read in the data considering only barcodes which passed `salmon`'s filter, you can supply the additional argument `alevinArgs = list(filterBarcodes=TRUE)` to the `tximeta()` function. 
Even if you do choose to read in pre-filtered data approach, it's still important to explore the data as we're about to do here and potentially filter further based on your observations, in particular since most mapping software's qualitu control measures (spoilers!) don't filter based on mitochondrial gene content. 
...
Again, it's important to take this step even if you have performed some initial filtering, since mapping software like `salmon`'s quality control does not consider mitochondrial read percentages.
