...
Just to check, we should have the same number of cells in `bladder_sce_filtered` and our previous `sc_counts_filtered`.
...
We could use our `detection_mat` to add up how many cells express each gene so we can try to filter out unreliably measured genes.  
...
How many genes will be excluded if we draw our cutoff at 10 cells?

```{r filter_effect, live = TRUE}
sum(num_samples_exp < 10)
```
...
# save as bladder_sce_filtered_gene_express
sc_counts_filtered_gene <- sc_counts_filtered[num_samples_exp >= cutoff, ]
```
...
```{r df_convert}
sc_filtered_df <- tibble::as_tibble(bladder_sce_filtered_gene_express) %>%
  dplyr::mutate(gene = rownames(bladder_sce_filtered_gene_express)) %>%
