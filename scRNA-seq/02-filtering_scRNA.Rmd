...
`tximeta` creates a `SummarizedExperiment` object (or more specifically a `RangedSummearizedExperiment` object.), which is used by many BioConductor packages to store and process results from gene expression studies.
...
`SingleCellExperiment` objects are a subtype of this that a lot of single-cell analysis R packages use, so we will try to get acquainted with them. 
