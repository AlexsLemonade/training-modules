...
mito_genes_df <- readr::read_tsv(mito_file) 
```
...
mito_rows <- rownames(sc_counts) %in% mito_genes_df$gene_id
...
filtered_samples <- qc_df %>%
  dplyr::filter(total_counts > 2000,
                num_genes_exp > 500,
                mito_fraction < 0.1)
...
Let's use our `detection_mat` to add up how many cells express each of these genes so we can try to filter out unreliably measured genes.  
