...
We want to remove cells with a low number of genes per cell, but not be too strict that we lose a large portion of cells from the dataset. 
...
```{r count_gene_cutoffs}
# Calculate the number of cells that would be removed with a given cutoff
gene_cutoff <- 500
sum(total_counts <= gene_cutoff)
```
