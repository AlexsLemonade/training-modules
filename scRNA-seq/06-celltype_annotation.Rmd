...
For details about how to perform these tasks with data that has been processed with Cell Ranger, you may want to look at the ["Integrating with protein abundance" chapter](http://bioconductor.org/books/3.22/OSCA.advanced/integrating-with-protein-abundance.html#setting-up-the-data) of OSCA.
...
nn_clusters <- scran::clusterCells(
  sce, 
  use.dimred = "PCA", # PCA input
  BLUSPARAM = bluster::NNGraphParam() # graph clustering & parameters
)
...
For more on that method, the OSCA section [Assigning cell labels from gene sets](http://bioconductor.org/books/3.22/OSCA.basic/cell-type-annotation.html#assigning-cell-labels-from-gene-sets) is a very good reference.
...
For more details about custom references, see the [OSCA chapter on cell type annotation](http://bioconductor.org/books/3.22/OSCA.basic/cell-type-annotation.html#using-custom-references)
