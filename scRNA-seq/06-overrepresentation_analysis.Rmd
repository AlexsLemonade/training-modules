...
* OSCA has [a section on using Gene Ontology sets to annotate clusters](https://bioconductor.org/books/release/OSCA/cell-type-annotation.html#assigning-cluster-labels-from-markers) using a different Bioconductor package.
...
`org.Hs.eg.db` is specific to _Homo sapiens_ -- this is what the `Mm` in the package name is referencing.
To perform gene identifier conversion in human (_Mus musculus_) we could use `org.Mm.eg.db`;
...
You can read a little more about this in the [_Invalidity of p-values_ section of the OSCA book](https://bioconductor.org/books/release/OSCA/marker-detection.html#p-value-invalidity).
...
Let's look at an UpSet plot of the significant results.

```{r cm_upsetplot, live = TRUE}
enrichplot::upsetplot(cellmarker_ora_results)
```
...
It's also likely that Kidney is not very relevant to our analysis of Hodgkins lymphoma, but Lymph node is highly relevant.
...
TODO: Section about not getting overly attached to the idea of significance and instead using this to help you interpret your clusters?
