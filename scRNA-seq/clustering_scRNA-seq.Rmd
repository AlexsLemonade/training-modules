...
are contributing
```{r}
VizPCA(object = seurat_norm, pcs.use = 1:2)
```
...
```{r}
PCElbowPlot(object = seurat_norm)
```
...
```{r}
# Finding gene markers with this clustering
markers <- find.markers(seurat_norm, 7, thresh.use = 2)
head(markers, 5)
```
