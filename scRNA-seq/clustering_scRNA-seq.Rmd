...
## Principal Components Analysis with high variance genes
...
(remember there are more genes that have 0's), we will do our PCA based on the 
...
Visualize the gene loadings for first two PC components.
What this shows us is that for most PC's there's a drop off for how much genes
are contributing
```{r}
VizPCA(seurat_norm, pcs.use = 1:2)
```

Let's visualize the standard deviations between PC's so we can determine which 
appear to summarize the information most informatively. 
```{r}
PCElbowPlot(seurat_norm)
```

Let's look at the distributions of the top 3 genes in the first PC. 
```{r}
VlnPlot(seurat_norm, c("ENSG00000173391", "ENSG00000115956", "ENSG00000135077"))
```
...
## tSNE experiment
tSNE is a popular dimension reduction technique that has become even more popular
with single cell rna-seq data analysis. 
It would be a good use of our time to explore tSNE parameters and how they 
affect the outcomes.
For our experiment, let's evaluate how various parameters, like perplexity,
affect tSNE outcomes.
...
run tSNE at different parameters and plot it with `ggplot2`. 
...
tSNE's output. 
However, using the cell-type labeling, we are finding that for this dataset tSNE
...
1) Different analyses (such as tSNE) have various strengths and limitations 
...
Let's visualize these clusters by making a dendrogram!
```{r}
# Make a dendrogram from the clustering
BuildClusterTree(seurat_norm)
```
