...
In this notebook, we'll perform quality control analyses of scRNA-seq count data. The [data set] (https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE84465)
we are using is cells from a glioblastoma sample  that were Fluorescence-Activated Cell sorted (FACS) and then processed by paired end sequencing of RNA using the [Smart-seq2 protocol](http://www.nature.com/doifinder/10.1038/nmeth.2639) 
...
We will discuss *tag-based* scRNA-seq data (such as 10X), in a later section, 
where you will learn how to process raw data to a count matrix and could apply
these same types of steps to filter and normalize the data.
...
The main files are further located in the `preprocessed` subdirectory (and sometimes deeper than that!). 

**Directory Structure diagram here**
...
This is actually a good idea/practice to enforce in your own projects, if you can.
