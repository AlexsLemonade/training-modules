...
We should check at this point that our data and metadata are in sync, which we have done in the past with `all.equal()`.
The sample ids in the metadata are stored in the `geo_accession` column.

```{r}
# get the sample names (every column but the first)
samples <- colnames(sc_data)[-1] 
# check that it matches the metadata
all.equal(samples, sc_metadata$geo_accession)
```

If they don't match, we can make line them up with some dplyr functions:
```{r}
 <- data.frame(samples) %>%
  dplyr::inner_join(sc_metadata, by  )
```
...
```{r}
sc_filtered_df <- tibble::as_tibble(sc_data_filtered) %>%
  dplyr::mutate(gene = rownames(sc_data_filtered)) %>%
...
```{r}
readr::write_rds(sc_filtered_df, filtered_count_file)
```
