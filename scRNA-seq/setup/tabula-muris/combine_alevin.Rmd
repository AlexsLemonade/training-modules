...
outfile <- file.path("..", "..", "data", "tabula-muris", "normalized", "TM_normalized.rds")
```
...
Here we are lso making sure we have a unique `cell` id that matches the format of `TM_droplet_metadata.csv` 
...
This may not be ideal, especially for the kidney samples which seem to have high overall mitochondrial percentages, but for now lets set the following cutoffs:
...
If we had some batch info, we might want to apply that to do better.
...
I am doing the Feature QC at this point because it does not use normalized data, and by my understanding of the normalization procedures, removing the low-expressed genes should not affect normalization. 
...
- Genes found in at least 10 samples
- mean count of at least 0.5
...
We will select the 1000 genes with the highest biologial variance across samples, after modeling the technical variance using `scran::modelGeneVar()`
