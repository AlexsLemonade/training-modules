...
From the table and the plot, we can see that the negative summary log fold change is probably driven by the very high expression values for Jchain in cluster 4.

Saa3 has a positive summary log fold change, let's take a look.
...
scater::plotReducedDim(normalized_sce, "UMAP", 
                       colour_by = "Saa3")
```
Though we see some individual samples with high expression values for Saa3 in cluster 2, it looks like it is most consistently highly expressed in cluster 7.
