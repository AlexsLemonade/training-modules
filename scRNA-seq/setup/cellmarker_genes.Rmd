...
cell_markers <- cell_markers %>%
  ...
  mutate(gene_symbol = str_remove_all(gene_symbol, "\\[")) %>%
...
cell_markers_list <- str_split(cell_markers$gene_symbol, 
                               pattern = ", ")
names(cell_markers_list) <- cell_markers$CellMarker

cell_markers_df <- reshape2::melt(cell_markers_list) %>%
  rename(gene_symbol = value, cell_marker = L1) %>%
  select(cell_marker, gene_symbol)
```
