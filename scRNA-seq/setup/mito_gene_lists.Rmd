...
```
We are interested in the latest annotation package, which as of this writing is Ensembl 99. The AnnotationHub query for that is then `AH78783`, which we can now dowload and save to a new object that is a Bioconductor Annotation DB.
```{r}
ensdb_hs <- ah[['AH78783']] 
```
...
Again, we will pick Ensembl 99, but we will skip saving the whole db  as its own object this time.
```{r}
