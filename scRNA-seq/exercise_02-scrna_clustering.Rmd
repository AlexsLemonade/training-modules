...
Note that there are other algorithms that we will not discuss, but can be explored in the [Flexible clustering for Bioconductor vignette](https://bioconductor.org/packages/3.22/bioc/vignettes/bluster/inst/doc/clusterRows.html).
...
For more discussion of the method in a single-cell context, including some tips on choosing `k`, the [Orchestrating Single-Cell Analysis book chapter on k-means](https://bioconductor.org/books/3.22/OSCA.basic/clustering.html#vector-quantization-with-k-means) is a good reference.
