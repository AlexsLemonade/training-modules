...
There are different approaches to filtering your data. 
In an ideal world, all RNA-seq data would come with the percent mapped reads 
statistics associated with it. 
This should be one of the first things you filter your data by.
Side note: even the highest quality single-cell RNA-seq data still has much 
lower percent mapped reads than bulk RNA-seq, so don't expect the same range of
percent mapped reads of bulk RNA-seq.
However, because a lot of publicly available data does not have percent mapped
read statistics associated with it, we are going to attempt to get around this
by trying other strategies of filtering first.
...
Regardless of what may have happened, we can't trust that 'cell'.
...
look for any biologically meaningful information. 
...
```
We got rid of some of the ridiculously low total count samples. 
...
ggplot(num_genes_exp_df, aes(x = num_genes_exp)) + 
  geom_density(aes(color = cell.type.ch1)) +
...
level, do we think that is biologically relevant, or is it more likely a 
technical artifact? 
...
ggplot(perc_map_reads, aes(x = percent_mapped_reads, fill = kept)) +
...
For this dataset, 79.5% of the data points are zeroes, which is not 
...
tricky.
So in order to normalize with less of these zeroes in the way, we normalize cells
in groups with other cells like them; a method determine by [Lun et al 2016](https://genomebiology.biomedcentral.com/articles/10.1186/s13059-016-0947-7).
...
3) Normalize using these pooled sum factors to create log counts per million reads (CPMs).  
...
Using a quick clustering, let's determine groupiungs for our cells that we will
normalize by.
...
```{r Remove negative size factor cells}
# Extract the negative factor data
neg.fact <- which(sce_raw@int_colData@listData$size_factor <= 0)

# Let's see how many cells this is
length(neg.fact)

# Since it's only three, let's go ahead and remove them from the data
sce_raw <- sce_raw[, -neg.fact]

# Take these out the metadata too
sc_metadata_filtered <- sc_metadata_filtered %>%
  dplyr::filter(geo_accession %in% sce_raw@colData@rownames)
```
...
One way to determine whether our normalization is yielding biolgically relevant 
...
objects and because we only really want
...
But overall, data seems moderately clustered by cell type, and for PC1 in 
...
gene_stats <- data.frame(log_var = log2(1 + gene_var), 
                         log_mean = log2(1 + gene_mean)) %>% 
