...
normalization. 
As you'll notice, the unnormalized, raw data has so many zeroes. 
...
all_means <- list("counts" = apply(seurat_norm@raw.data, 1, mean), 
                  "log_norm" = apply(seurat_norm@scale.data, 1, mean))
...
are highly expressed, but don't change much between different biological 
conditions (think housekeeping-like genes). 
...
for each gene.
We'll also create a variable that keeps track of whether a gene was considered
high or low variance according to the `Seurat::FindVariableGenes`.
...
ggplot(var_stats, aes(x = gene.mean, y = gene.dispersion.scaled, color = var_group)) +
  geom_point(alpha = 0.15) +  # We are using alpha so that the points are more see-through
  xlab("Gene log(Mean)") +
  ylab("Gene log(Variance/Mean)") + 
  theme_cowplot() +
  guides(colour = guide_legend(override.aes = list(alpha = 1))) + 
  geom_vline(xintercept = .1, size = .5, alpha = .4) +  # Here we are plotting our variance cutoff
  geom_hline(yintercept = 1, size = .5, alpha = .4)  # Here we are plotting our expression cutoff
```
