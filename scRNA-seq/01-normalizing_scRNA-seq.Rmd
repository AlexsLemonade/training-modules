...
You'll notice the rows contain gene information and the columns contain sample information. 
For the analyses in this notebook, we will not be using gene identifiers, so let's remove 
that first column. 

```{r Remove the gene column}
# We don't need the gene column
sc_data <- sc_data[, -1]
```

Let's look at how the gene means look in this dataset. 
We will use `apply` in order to calculate things across our data. 
Note the second argument is to specify whether to calculate by rows or columns. 
(1 = rows, 2 = columns).
...
# Plot the means using ggplot2's `qplot`
...
Due to the high risk of technical artifacts, we don't necessarily want 
...
seeing how many counts were measured in total. 
Let's take a look at this data from how many counts are there per sample. 
Remember samples are by columns, so we are using `2` here. 
...
This is why exploring and filtering your data prior to performing analysis is 
important.
Let's visualize the distribution of total counts to see if the 48 count sample 
is the only one we should get rid of.

For the following graphs, we will use vertical red lines to indicate possible 
cutoffs. 
We will learn more about `ggplot2` later on, but note that layers and extra 
features of a plot are added with `+` like below.

```{r Explore total counts distribution}
# Let's take a look at what this looks like:
qplot(total_counts, geom = "density", xlab = "gene mean counts") +
...
It was good to plot cell types separately. 
That way we know how this may be affecting different cell types differently. 
...
It's best just to get rid of these genes that we suspect are not being reliably 
measured by many cells. 
...
You can find more details on the normalization step [here](https://bioconductor.org/packages/release/bioc/vignettes/scran/inst/doc/scran.html)
...
in this instance, we would like to have PCA scores for each cell.
