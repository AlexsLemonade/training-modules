...
We will load in this pre-made function using the `source` function. 
...
![**Overall view of full-length and tag-based workflows**](figure/full-length_1.png) 
...
Besides, the typical R object types, special object types are generally specific 
to groups of R packages. 
`SingleCellExperiment` objects are what a lot of single-cell analysis R packages
use, so we will try to get acquainted with them. 
...
Here's a modified version of the figure from [Amezquita et al. _bioRxiv._ 2019.](https://www.biorxiv.org/content/10.1101/590562v1) 
that shows the general structure of `SingleCellExperiment` objects. 
Note that each function designed to transform the `SingleCellExperiment` object
also has a corresponding function that allows you to extract (or setting)
the information from that object. 

![SingleCellExperiment Object Structure](figure/sce_structure.png)
...
```{r}
readr::write_tsv(sc_metadata_filtered, file.path(norm_dir, "gbm_filtered_metadata.tsv"))
```
