...
Now we can make some plots of the QC data and use it to inform our next steps/
...
Now we will perform the same normalization steps we did with the Smart-seq2 data, using `scran`scran::computeSumFactors()` and `scater::logNormCounts()`
...
These results, with accompanying metadata were saved into a `SingleCellExpriment` object, which is stored as RDS file at the location `tm_normalized_file` (defined in the `filepath` chunk above).
...
Another dimentionality technique that you may have heard of is **t-SNE**, which has similar properties to UMAP, and often produces similar results. 
...
Regardless of why, in our experience, UMAP seems to produce slightly better results, but the differences are relatively subtle.
...
UMAP can be very slow for a large data set with lots of parameters.
...
Now lets convert that matrix result to a data frame and plot the results!
...
Lets add in the metadata we have stored in the `SingleCellExperiment` data (colData) and replot it.
...
```{r plot_color}
ggplot(data.frame(tm_meta_umap), 
...
```{r UMAP-function}
UMAP_plot_wrapper <- function(nn_param = 15, sce = tm_normalized) {
...
Lets make sure that works and gives the same result as before when we use the default parameters
```{r function-test}
...
We can use `set.seed()` to be sure that an individual run (or set of runs) is the same every time, but it is important to check your results a few times to be sure that the random component is giving you anomalous results.



Fill in the next five code chunks with the function and the `n_neighbors` argument you would like to use for each. 
...
1) Different analyses (such as UMAP) have various limitations for interpretability. The coordinates of UMAP output for any given cell can change dramatically depending on parameters, and even run to run with the same parameters. This probably means that you shouldn't rely too heavily on the exact values of UMAP's output. 
2) Different analyses also have their strengths. Using cell-type labeling, our experiment illustrated that UMAP does appear to give some biologically relevant output information for this dataset. 
