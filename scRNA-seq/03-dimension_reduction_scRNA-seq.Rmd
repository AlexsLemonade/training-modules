...
the same kinds of steps we did in `01-normalizing_scRNA-seq.Rmd`).
We also randomly subsampled 500 cells, and are only using the top 95% highest 
average expression genes.

```{r Load in Tabula Muris Data}
# Read in gene matrix
tab_mur_data <- readr::read_tsv(file.path("data", "scran_norm_tab_mur.tsv"), 
                                progress = FALSE) 
...
Just like we did with our other scRNA-seq data, we will also run PCA with our 
single-cell data. 
...
transposed_data <- t(tab_mur_data_sub)
```
...
Remember that previously we set up `tab_mur_meta` to have the corresponding
