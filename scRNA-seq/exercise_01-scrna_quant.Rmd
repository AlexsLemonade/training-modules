...
(This chunk won't work if you try to run it due to permissions errors; no worries.)
...
If you completed Part A of this exercise notebook, you will have quantified only a subset of reads from this sample.
For this section, we have prepared the quantified data from the full set of reads, allowing you to look at more representative results for a sample.

This quantified data is available for you to use in the directory `data/tabula-muris/alevin-quant/10X_P7_12`.
...
You can read more about doublet detection in the [Orchestrating Single Cell Analysis book](https://bioconductor.org/books/3.22/OSCA.advanced/doublet-detection.html) and find more detailed information on using the `computeDoubletDensity` function in the [bioconductor vignette](https://bioconductor.org/packages/3.22/bioc/vignettes/scDblFinder/inst/doc/computeDoubletDensity.html).
...
In the chunk below, perform PCA on the filtered SCE object using `scater::calculateCA()` and save the result to a variable called `tm_pca`.
...
We'll prepare the data for this plot by joining the doublet scores with the PCA result we stored in `tm_pca`.
...
Now, you can plot the `PC1` on the x-axis with `PC2` on the y-axis, coloring the data points by the `DoubletScore`.
