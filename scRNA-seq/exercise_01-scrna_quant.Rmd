...
# Write out the file path for 10X_P7_12's R2 FILE 002 file. 
...
We will want to store the output in the `alevin-quant` folder, with its own folder named as `10X_P7_12`.
...
cp -r ~/shared-data/training-data/tabula-muris/alevin/10X_P7_12 data/tabula-muris/alevin-quant/.
```
...
```{r solution=TRUE}
# Specify the file path to the `quants_mat.gz` file for 10X_P7_12 and call this object `quant_file`.
