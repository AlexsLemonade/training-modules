...
*Note*: depending on the state of the data you are working with, i.e., if you have a set of `.bcl` files, you will need to use [`cellranger mkfastq`](https://support.10xgenomics.com/single-cell-gene-expression/software/pipelines/latest/using/mkfastq) to make `.fastq` files from this.
...
Note that for the first section 
...
#### `-l`
The `-l` option is for designating library format 
...
Drop-seq data is also supported, and in this case you would use a `--dropseq` flag instead of this.
