...
We will do this modeling of variance by expressionwith the `scran::modelGeneVar()` function, saving the results to a new variable. 

```{r model_variance}
gbm_variance <- scran::modelGeneVar(gbm_sce)
```

Now lets plot the relationship between gene expression and variance we were discussing.
...
We can add a function like that curveto a `ggplot` with a `stat_function` layer.
...
Since each of the tables in the list-like object has a name, we can select an individual table using the familiar `$` syntax.
...
We will introduce `purrr` briefly here, but if you want more information and background, have alook at the [`purrr` cheatsheet (PDF)](https://github.com/rstudio/cheatsheets/raw/master/purrr.pdf) and Jenny Bryan has a a great [`purrr` tutorial](https://jennybc.github.io/purrr-tutorial/index.html) you might want to look at.
...
The first one, `purrr::map()` is essentially equivilent to the `lapply()` function that is part of base R, but with some different defaults. 
We will use it first to get the top rows from each table by using it to apply the `head()` function to each element of the list.
...
Since that would return a list of data frames, it isn't quite what we want.
There is no built-in function that will give us just the first few rownames, so we will have to define one, and `purrr` gives us a nice shorthand for doing that with `~` syntax.
Each element of the list will be assigned to the `.x` variable, so we can define an expression that uses that and `purrr::map()` will apply that to each element of the list.

```{r head_markernames, live = TRUE}
# Get the first few rownames of each table with a purrr function.
purrr::map(as.list(markers), # convert markers to a 'regular' list for purrr
...
It's special purpose is that after it applies the operations in the second argument, it combines the results into a single data frame.
