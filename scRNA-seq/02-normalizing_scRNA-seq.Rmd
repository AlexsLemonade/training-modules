...
glio_sce <- SingleCellExperiment(list(counts = as.matrix(sc_data_filtered[, -1])))

# Store the gene names in this object
rownames(glio_sce) <-  sc_data_filtered$gene
colnames(glio_sce) <- colnames()
```
...
sample_info <- tibble::tibble(geo_accession = colnames(glio_sce)) %>%
...
all.equal(sample_info$geo_accession, colnames(glio_sce))
```
...
```{r}
colData(glio_sce) <- DataFrame(sample_info, row.names = sample_info$geo_accession)
```
...
qclust <- scran::quickCluster(glio_sce)

# Step 2) Compute sum factors for each cell cluster grouping.  
glio_sce <- scran::computeSumFactors(glio_sce, clusters = qclust)

# Step 3) Normalize using these pooled sum factors and log transform. 
glio_sce <- scater::logNormCounts(glio_sce)
```
...
log_pca <- t(counts(glio_sce)) %>% # transpose the count matrix
...
log_pca_scores <- data.frame(log_pca$x[, 1:2],
                            ...
                            cell_type = colData(glio_sce)$cell_type)
...
subset_pca <- t(logcounts(glio_sce)[highvar_genes,]) %>%
  prcomp()
```
...
to a tsv file. 
In order to do this we need to extract our normalized counts from `glio_sce`. 
...
We will return to our normalized `glio_sce` object in the exercise, so we will 
...
readr::write_rds(glio_sce, file.path(norm_dir, "glioblastoma_sce.RDS"))
```
