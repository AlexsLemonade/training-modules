...
Now what we didn't tell you before, but will tell you now, is that the default for `scater::calculatePCA()` is to use the top 500 highest variance genes. 
...
`The highest variance genes may not always be the most informative on their own.

``{r find_markers}
markers <- fscran::indMarkers(gbm_sce, gbm_sce$cell_type)
...
```
Get the top markers for each cell type using `purrr`
```{r top_markers}
...
```{r}
marker_pca <- calculatePCA(gbm_sce, subset_row=unique_markers) %>%
  as.data.frame() %>%
...
ggplot(marker_pca, aes(x = PC1, y = PC2, color = cell_type)) +
  geom_point() + 
  ggtitle("PC scores for marker genes") + 
  colorblindr::scale_color_OkabeIto()
```
