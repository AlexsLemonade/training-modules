...
If you notice there will be files that contain either `R1` or `R2` within the filename.
Tag-based single-cell sequencing is sequenced using paired end sequencing, which means you will have sequence information from both ends of each read. 
For 10X data, files with `R1` will contain the cell barcode and UMI sequence, while files with `R2` will have the cDNA sequence.
...
Note that here we are only giving the full paths to 1 of the `R1` files and 1 of the `R2` files. 
...
The fraction of reads in whitelist barcodes is particularly important as a low percentage here means the library contains reads that do not correspond to cell barcodes, indicating poor single-cell capture. 
The mean number of reads per cell and median number of detected genes per cell can be helpful in understanding how deep the library was sequenced, as the higher these numbers are, the more information you will obtain per cell. 

The `Knee plot` shows the frequency of each barcode being detected and the estimation of the `knee` cutoff. 
Alevin uses knee-based filtering and only cells above this `knee` threshold will be considered true cells and be included in the final cell barcode whitelist.
