...
The `knee plot` shows all possible cell barcodes identified in the library. 
The y-axis shows the number of distinct UMIs for each cell barcode.
Cells are then ranked by the number of UMIs and rank is depicted on the x-axis.

Cell barcodes with low UMI counts are considered empty droplets. 
These droplets must be filtered out so we only consider true cells for downstream analysis.

The `knee` or inflection point is identified on the curve, and only cells above this threshold will be included in the final cell barcode whitelist.

Alevin, and [`cellranger count`](https://support.10xgenomics.com/single-cell-gene-expression/software/pipelines/latest/using/count), both use knee-based filtering.  
