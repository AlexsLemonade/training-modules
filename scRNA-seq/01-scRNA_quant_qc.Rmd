...
```bash
mkdir -p data/tabula-muris/alevin-quant/10X_P7_14_subset
```

In this scenario, `10X_P7_14` refers to the sample name that we will be processing this time, which contains data from mouse limb muscle cells.
...
This step would be the same for mapping bulk RNA-seq data, and you can use the same transcriptome indexes as bulk RNA-seq, however, due to the shorter read lengths in the 10X sequencing, we may to use shorter kmers than the default index size that salmon uses.
...
salmon alevin \
  -i index/Mus_musculus/short_index \
  -l ISR \
  -1 data/tabula-muris/fastq/10X_P7_14/10X_P7_14_L001_R1_001.fastq.gz \
  -2 data/tabula-muris/fastq/10X_P7_14/10X_P7_14_L001_R2_001.fastq.gz \
  -o data/tabula-muris/alevin-quant/10X_P7_14_subset \
  -p 4 \
  --chromium  \
...
For 10X data, the two read files are organized as follows:
- `R1` files contain the barcodes for cells as well as the UMIs
- `R2` files contain the RNA-seq reads for that sample.  
...
This was the case for the *Tabula Muris* data we used here as well. If you look in the `data/tabula-muris/fastq/10X_P4_3` directory, you will see the following files, representing 2 lanes worth of data, with three R1 and three R2 files per lane:

```
10X_P7_14_L001_R1_001.fastq.gz  10X_P7_14_L002_R1_001.fastq.gz
10X_P7_14_L001_R1_002.fastq.gz  10X_P7_14_L002_R1_002.fastq.gz
10X_P7_14_L001_R1_003.fastq.gz  10X_P7_14_L002_R1_003.fastq.gz
10X_P7_14_L001_R2_001.fastq.gz  10X_P7_14_L002_R2_001.fastq.gz
10X_P7_14_L001_R2_002.fastq.gz  10X_P7_14_L002_R2_002.fastq.gz
10X_P7_14_L001_R2_003.fastq.gz  10X_P7_14_L002_R2_003.fastq.gz
```
...
#   -i index/Mus_musculus/short_index \
#   -l ISR \
#   -1 data/tabula-muris/fastq/10X_P7_14/10X_P4_3_L*_R1_*.fastq.gz \
#   -2 data/tabula-muris/fastq/10X_P7_14/10X_P4_3_L*_R2_*.fastq.gz \
#   -o data/tabula-muris/alevin-quant/10X_P7_14 \
#   -p 4 \
#   --chromium  \
#   --tgMap index/Mus_musculus/Mus_musculus.GRCm38.95.versioned_tx2gene.tsv \
#   --dumpFeatures
...
alevinQC::alevinQCReport("data/tabula-muris/alevin-quant/10X_P4_3_subset",
                         sampleId = "10X_P7_14_subset",
                         outputFile = "10X_P7_14_subset-qc_report.html",
