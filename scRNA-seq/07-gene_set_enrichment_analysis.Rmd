...
Individual cells may also differently sized (Blasi *et al.* 2017), be at different stages of the cell cycle ([Satija lab](https://satijalab.org/seurat/archive/v3.1/cell_cycle_vignette.html)), or experiencing different levels of cellular stress ([Luecken and Theis. 2019](https://doi.org/10.15252/msb.20188746)), some of which may be interesting in the context of cancer and therefore should not be corrected for.
...
If many genes "drop out" between cell types, we may not expect to capture subtle signals between clusters.

With all this in mind, it may be unsurprising that ORA, which generally captures genes with large individual changes, is well-suited to analyzing scRNA-seq data and the advantages of GSEA in a bulk analysis setting may not be fully available here. 
...
As such, this statistic gives us information about the magnitude and directionality of a gene's expression in cluster 1 vs. all other clusters.
...
The gene set `GSE29618_BCELL_VS_PDC_DAY7_FLU_VACCINE_DN` had a highly negative NES.
...
Let's look at `GOLDRATH_EFF_VS_MEMORY_CD8_TCELL_DN`, which was not in the results we viewed earlier.
