...
marker_dir <- file.path("analysis", "tabula-muris", "markers")
...
In a previous notebook, we used some of the data from the Tabula Muris project analysis to tag our cells for visualization.
...
cell_metadata <- sc_metadata %>%
  ...
  # extract 16 nt cell barcodes from the `cell` column
...
Now we can add that data back to the SCE object.
...
Note that this will replace all of the previous column (cell) metadata, which is why we had to pull out the previous content first.
...
We will return to our normalized `bladder_sce` object in the exercise, so we will 
...
readr::write_rds(gbm_sce, file.path(norm_dir, "normalized_bladder_sce.rds"))
```
