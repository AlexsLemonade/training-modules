...
This method, described in [Lun *et al.* (2019)](https://doi.org/10.1186/s13059-019-1662-y), uses the droplets with very low UMI counts to estimate the "ambient" expression pattern of RNA content outside of cells.
...
![**Status of the current dataset**](diagrams/cellranger_2.png)
...
This is the default approach that `runPCA()` and `calculatePCA()` take, using 500 genes with the greatest variance across cells to calculate the PCA matrix. 
This approach also makes the PCA calculations somewhat faster.
...
Here we are picking the 500 top genes, to match the number of genes we used before.
...
You might realize that our old PCA values were replaced when we ran `runPCA()` again.
