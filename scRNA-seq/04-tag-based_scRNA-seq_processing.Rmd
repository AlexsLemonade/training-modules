...
```
mkdir -p data/tabula-muris/alevin/10X_P4_3
```
...
```
salmon alevin -l ISR \
  -i ~/shared-data/reference/refgenie/mm10_cdna/salmon_index/short \
  ...
  --chromium  \
  -p 5 \
  -o data/tabula-muris/alevin/10X_P4_3 \
...
For our example, we've pre-made the file `Mus_musculus.GRCm38.95.versioned_tx2gene.tsv` from the Ensembl transcriptome that we indexed above. 
...
This was the case for the *Tabula Muris* data we used here as well. If you look in the `data/tabula-muris/fastq/10X_P4_3` directory, you will see the following files, representing 2 lanes, with three R1 and three R2 files per lane:
...
#   --chromium  \
#   -p 4 \
#   -o data/tabula-muris/alevin/10X_P4_3_full \
#   --tgMap index/Mus_musculus/Mus_musculus.GRCm38.95.versioned_tx2gene.tsv \
#   --dumpFeatures
```
...
# Produce a QC report
alevinQC::alevinQCReport("data/tabula-muris/alevin/10X_P4_3",
                         sampleId = "10X_P4_3",
...
quant_dir <- file.path("data", "tabula-muris", "alevin", "10X_P4_3")
