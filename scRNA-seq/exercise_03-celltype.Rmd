...
Next, we'll create a UMAP plot, coloring the cells by the identified cell types now present in the `celltype_hpca` column of the `colData`.

But before we do...there's quite a few cell types here that most default palettes will struggle to accommodate.
To help with this, we've created a custom palette to use with this plot that matches the colors you see above in the `SingleR` heatmap.

```{r create hpca palette}
hpca_cell_types <- sort(unique(hpca_result$pruned.labels))

# colors originally derived from dittoSeq: https://github.com/dtm2451/dittoSeq
# this vector of colors was obtained from: https://github.com/SingleR-inc/SingleR/blob/40ff41ae2387b3de6673ea516f437266caa08ace/R/plotScoreHeatmap.R#L576-L591
hpca_cell_palette <- c("#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2",
                      "#D55E00", "#CC79A7", "#666666", "#AD7700", "#1C91D4",
                      "#007756", "#D5C711", "#005685", "#A04700", "#B14380")

names(hpca_cell_palette) <- hpca_cell_types
```


```{r hpca plot cell types}
# plot UMAP with HPCA cell types
scater::plotUMAP(sce, color_by = "celltype_hpca") + 
  # specify the custom palette
  scale_color_manual(
    values = hpca_cell_palette, 
    na.value = "gray80", # set a light color to distinguish unlabeled cells
    name = "Cell type"
  )
```
...
The full list of datasets available in the package can be found in [this `scRNAseq` vignette](https://bioconductor.org/packages/3.22/data/experiment/vignettes/scRNAseq/inst/doc/scRNAseq.html#finding-datasets).
...
This method is more appropriate for [annotation with a single-cell reference dataset](https://bioconductor.org/books/3.22/SingleRBook/sc-mode.html#annotation-with-test-based-marker-detection) because it accommodates the low coverage and sparsity commonly seen in single-cell data.
...
See the section on [choosing assay data from the `SingleR` book](https://bioconductor.org/books/3.22/SingleRBook/classic-mode.html#choices-of-assay-data) for more details.
...
brain_model <- SingleR::trainSingleR(
  ...
  test.genes = rownames(sce), # only use genes in the sce object
...
To help interpret the figure, we'll again define a custom palette, this time called `brain_cell_palette`, to match the heatmap above that you can use when plotting the UMAP.

```{r create brain palette}
brain_cell_types <- sort(unique(brain_result$pruned.labels))

# colors obtained from: https://rdrr.io/bioc/SingleR/src/R/plotScoreHeatmap.R
brain_cell_palette <- c("#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")

names(brain_cell_palette) <- brain_cell_types
```
...
We also see that the cluster of astrocytes from the HPCA annotation appears to be a mix of several cell types, including neurons across developmental stages (`fetal_quiescent` and `fetal_replicating`). 
Do these results change our answer to the previous question of which of the 4 glioblastoma cell states are present in our sample?
...
```{r metacell umap}
# create hpca umap
hpca_umap <- scater::plotUMAP(
  sce,
  color_by = "hpca_metacell"
)

# create brain umap
brain_umap <- scater::plotUMAP(
  sce,
...
```{r marker expression, fig.width = 12, fig.height = 5}
