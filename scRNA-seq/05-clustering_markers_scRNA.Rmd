...
There are two main parts for the NN clustering algorithm: the network building and then the community finding.
The network building 


```{r}
clusterPlot(hodgkins_sce, NNGraphParam(k = 20, cluster.fun = "louvain", type = "jaccard"))
```
...
Because we don't know the identies of the clusters we identified, these are just the cluster numbers for now.
