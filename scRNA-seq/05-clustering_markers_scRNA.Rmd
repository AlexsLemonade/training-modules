...
Because, clustering algorithms like k-means can be a bit slow with as many features as the number of genes that we have in our data set, so we would rather not use the raw data.
...
The other argument we need for `scran::clusterCells()` is will tell it which clustering algorithm to use, and any additional parameters associated with that algorithm.
...
kclusters <- scran::clusterCells(
  hodgkins_sce, # the SCE object to cluster
  use.dimred = "PCA", # use the PCA reduced dimension for clustering
  BLUSPARAM = KmeansParam(centers = k) # bluster parameter to provide the clustering algorithm
)

kclusters <- scran::clusterCells(
  sce, # the SCE object to cluster
  use.dimred = "PCA", # use the PCA reduced dimension for clustering
  BLUSPARAM = bluster::KmeansParam(centers = k) # bluster parameter to provide the clustering algorithm
)
```

The `scran::clusterCells()` function returned a vector of cluster assignments as integers.
It has also encoded this vector as a factor, so R will treat these integers are categories and not as a continuous variable.
...
hodgkins_sce$kcluster <- kclusters
```
...
nnclusters <- scran::clusterCells(
  hodgkins_sce,
  use.dimred = "PCA",
  BLUSPARAM = bluster::NNGraphParam(k = 10,
                  cluster.fun = "walktrap")
)
# store cluster results in the SCE object
hodgkins_sce$nncluster <- nnclusters
```
