...
The `gs_collection` and `gs_subcollection` columns of `hs_msigdb_df` contain the collection information.
...
Hint: `hs_msigdb_df` (and therefore the subsetted data frame you saved to `gene_set_df`) stores Entrez ids in a column called `ncbi_gene`.
You'll need to specify this column when forming your `TERM2GENE` argument to ensure the same kinds of ids are used throughout the analysis.
