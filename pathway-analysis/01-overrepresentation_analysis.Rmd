...
The simplicity of only requiring an input gene set (sort of, more on that below) can be attractive, especially if we want to do something beyond a two group comparison.
...
So we will need to convert from these identifiers into either gene symbols or Entrez IDs.
...
TODO: Say more about the study here!!! 
...
TODO: Link to setup notebook!

The way we set up the analysis (see `setup`), genes with positive log fold-changes (LFC) should be more highly expressed in the high stem cell capacity cells. 
...
Although we're picking a _commonly used_ cutoff (padj < 0.05), it's still arbitrary and we could just as easily pick a different threshold for our LFC values.
...
Now, we'll basically do the same thing for our other results. 
...
We can see that some of the DNA repair KEGG pathways share genes.
We need to be careful in our interpretation of our pathway results, as sometimes multiple pathways coming up is mostly an indication of a handful of genes in our gene list.
