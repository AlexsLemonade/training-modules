...
library(msigdbr)  # TODO: install from CRAN, we also need ggupset!
...
We will use gene sets from the [Molecular Signatures Database (MSigDB)](https://www.gsea-msigdb.org/gsea/msigdb/index.jsp) from the Broad Institute ([Subramanian, Tamayo *et al.* (2015)](https://doi.org/10.1073/pnas.0506580102)). 
...
We can see what types of IDs are available to us with `keytypes()`.
...
Maybe you're thinking, "Hey, picking the top 100 genes seems kind of arbitrary."
You are correct!
...
In the scRNA-seq module (`01-filtering_scRNA-seq.Rmd` specifically), we filtered out genes that were expressed in less than 200 cells, but all genes that were detected should be in our marker data frame.
